 ...
 */
#define DEBUG  1
#define FEATURE_PM_MANAGED_VOLTAGE 
...
#define BATTERY_SET_MOVINGMODE_PROC				17
...
#define ONCRPC_CHG_XO_THERM_ADC_READ_PROC 	22
 

 
#define ONCRPC_CHG_USB_OVP_NOTIFY_PROC 	23
 

#define ONCRPC_CHG_USB_OVPRECV_NOTIFY_PROC 	24
...
#define BATT_WLOCK_SEC_CUTOFF  (100)

 



#define BATT_WLOCK_SEC_CHARGER (10)
...
boolean ovp_pending_status = FALSE;
boolean ovp_pending_data = FALSE;



struct wake_lock oem_msm_batt_wake_lock;




#define		PM_MOVING_1X_EVDO_MODE		0x00000001
#define		PM_MOVING_CAMERA_MODE		0x00000002
#define		PM_MOVING_CONNECT_MODE		0x00000004
#define		PM_MOVING_CAMLIGHT_MODE		0x00000008
#define		PM_MOVING_BACKLIGHT_MODE	0x00000010
#define		PM_MOVING_KEY_MODE			0x00000020
#define		PM_MOVING_VIB_MODE			0x00000040
#define		PM_MOVING_SPK_MODE			0x00000080
#define		PM_MOVING_BT_MODE			0x00000100
#define		PM_MOVING_WLAN_MODE			0x00000200
#define		PM_MOVING_SENSOR_MODE		0x00000400




 
#define		PM_MOVING_DTV_MODE		        0x00001000
#define		PM_MOVING_FELICA_MODE	     	0x00002000
 


struct msm_batt_set_movingmode_data {
	pm_obs_mode_type index;
	u32 value;
};
...
typedef struct {
	...
	struct msm_batt_set_movingmode_data				connect_1x_data;	
	struct msm_batt_set_movingmode_data				camera;				
	struct msm_batt_set_movingmode_data				connect;			
	struct msm_batt_set_movingmode_data				cam_light;			
	struct msm_batt_set_movingmode_data				backlight_level;	
	struct msm_batt_set_movingmode_data				key_light;			
	struct msm_batt_set_movingmode_data				vib;				
	struct msm_batt_set_movingmode_data				spk;				
	struct msm_batt_set_movingmode_data				bt;					
	struct msm_batt_set_movingmode_data				wlan;				
	struct msm_batt_set_movingmode_data				sensor;				




     
	struct msm_batt_set_movingmode_data				dtv;				
	struct msm_batt_set_movingmode_data				felica;				
     

} pm_obs_movingmode_data_type;

static pm_obs_movingmode_data_type pm_obs_movingmode ={
	.bit = 0x0,
};

static void msm_batt_set_movingmode_init(void);
				       ...
				       void *buf, void *data)
{
	...
	data_ptr->battery_per = be32_to_cpu(buf_ptr->battery_per);
...
static u32 msm_batt_get_vbatt_voltage(u32 *per)
{
	int rc;
...
static void msm_batt_update_psy_status(void)
{
	...
	battery_voltage = msm_batt_get_vbatt_voltage(&battery_per);
	...
	if (charger_status == msm_batt_info.charger_status &&
		...
		msm_batt_info.batt_capacity == battery_per &&
	...
	if (msm_batt_info.charger_type != charger_type) {
		if (charger_type == CHARGER_TYPE_USB_WALL ||
			...
			if (batt_wlock_sec < BATT_WLOCK_SEC_CHARGER) {
				batt_wlock_sec = BATT_WLOCK_SEC_CHARGER;
			}
	...
	if (battery_per <= 0) {
		if (batt_wlock_sec < BATT_WLOCK_SEC_CUTOFF) {
			batt_wlock_sec = BATT_WLOCK_SEC_CUTOFF;
		}
	}
	...
	if (msm_batt_info.battery_status != battery_status) {
		if (battery_status != BATTERY_STATUS_INVALID) {
			...
			} else if (battery_status == BATTERY_STATUS_OVP) {
				DBG_LIMIT("BATT: Battery OverVoltage.\n");
				msm_batt_info.batt_health =
	...
	if (supp) {
		...
		if (batt_wlock_sec > 0) {
			wake_lock_timeout(&oem_msm_batt_wake_lock, (batt_wlock_sec * HZ));
		    pr_info("%s: wake_lock timeout=%ldsec\n", __func__, batt_wlock_sec);
		}
			     ...
			     u32 voltage_direction, u32 batt_cb_id, u32 cb_data)
{
	...
	wake_lock_init(&oem_msm_batt_wake_lock, WAKE_LOCK_SUSPEND, "oem_msm_batt");
...
static int msm_batt_deregister(u32 batt_handle)
{
	...
	wake_lock_destroy(&oem_msm_batt_wake_lock);
...
static int __init msm_batt_init(void)
{
	...
	if( ovp_pending_status == TRUE )
	{
		
		pm_usb_ovp_notify(ovp_pending_data);
	}



	msm_batt_info.boot_flg = TRUE;

	msm_batt_set_movingmode_init();
...
static int msm_batt_set_movingmode_client_arg_func(struct msm_rpc_client *batt_client, void *buf, void *data)
{
	struct msm_batt_set_movingmode_data *ptr = (struct msm_batt_set_movingmode_data *)data;
	u32 *req = (u32 *)buf;
	int size = 0;

	*req = cpu_to_be32(ptr->index);
	size += sizeof(u32);
	req++;
	
	*req = cpu_to_be32(ptr->value);
	size += sizeof(u32);

	return size;
}

static void msm_batt_set_vbatt_mode( struct msm_batt_set_movingmode_data *data )
{
	int rc;

	rc = msm_rpc_client_req(msm_batt_info.batt_client,
			BATTERY_SET_MOVINGMODE_PROC,
			msm_batt_set_movingmode_client_arg_func, data,
			NULL, NULL,
			msecs_to_jiffies(BATT_RPC_TIMEOUT));

	if (rc < 0) {
		pr_err("%s: FAIL: vbatt set movingmode rc=%d\n", __func__, rc);
		return;
	}
...
{
	struct msm_batt_set_movingmode_data data;

	data.index = mode;
	data.value = (u32)sw;

	if (msm_batt_info.boot_flg)
	{
		msm_batt_set_vbatt_mode( &data );
	} else {
		
		pm_obs_movingmode.bit |= PM_MOVING_CAMERA_MODE;
		pm_obs_movingmode.camera = data;
	}
}
...
void pm_obs_a_camlight(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_CAMLIGHT_MODE;
		pm_obs_movingmode.cam_light = data;
	}
}

void pm_obs_a_lcdbacklight(u32 level)
{
	struct msm_batt_set_movingmode_data data;

	data.index = PM_OBS_LCDBACKLIGHT_MODE;
	data.value = level;
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_BACKLIGHT_MODE;
		pm_obs_movingmode.backlight_level = data;
	}
}
...
void pm_obs_a_keybacklight(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_KEY_MODE;
		pm_obs_movingmode.key_light = data;
	}
}
...
void pm_obs_a_vibration(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_VIB_MODE;
		pm_obs_movingmode.vib = data;
	}
}

void pm_obs_a_speaker(u32 value)
{
	struct msm_batt_set_movingmode_data data;

	data.index = PM_OBS_SPEAKER_MODE;
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_SPK_MODE;
		pm_obs_movingmode.spk = data;
	}
}
...
void pm_obs_a_bluetooth(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_BT_MODE;
		pm_obs_movingmode.bt = data;
	}
}
...
void pm_obs_a_wlan(pm_obs_mode_wlan_type mode)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_WLAN_MODE;
		pm_obs_movingmode.wlan = data;
	}
}
...
void pm_obs_a_sensor(pm_obs_mode_sensor_type mode)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_SENSOR_MODE;
		pm_obs_movingmode.sensor = data;
	}
}
...
void pm_obs_a_dtv(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_DTV_MODE;
		pm_obs_movingmode.dtv = data;
	}
}
...
void pm_obs_a_felica(pm_obs_mode_type mode, boolean sw)
{
	...
	if (msm_batt_info.boot_flg)
	{
		...
		pm_obs_movingmode.bit |= PM_MOVING_FELICA_MODE;
		pm_obs_movingmode.felica = data;
	}
}
...
static void msm_batt_set_movingmode_init(void)
{
	...
	if (pm_obs_movingmode.bit & PM_MOVING_CAMERA_MODE)
	{
		pm_obs_a_camera( pm_obs_movingmode.camera.index, (boolean)pm_obs_movingmode.camera.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_CAMERA_MODE;
	}
	...
	if (pm_obs_movingmode.bit & PM_MOVING_CAMLIGHT_MODE)
	{
		pm_obs_a_camlight( pm_obs_movingmode.cam_light.index, (boolean)pm_obs_movingmode.cam_light.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_CAMLIGHT_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_BACKLIGHT_MODE)
	{
		pm_obs_a_lcdbacklight( pm_obs_movingmode.backlight_level.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_BACKLIGHT_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_KEY_MODE)
	{
		pm_obs_a_keybacklight( pm_obs_movingmode.key_light.index, (boolean)pm_obs_movingmode.key_light.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_KEY_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_VIB_MODE)
	{
		pm_obs_a_vibration( pm_obs_movingmode.vib.index, (boolean)pm_obs_movingmode.vib.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_VIB_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_SPK_MODE)
	{
		pm_obs_a_speaker( pm_obs_movingmode.spk.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_SPK_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_BT_MODE)
	{
		pm_obs_a_bluetooth( pm_obs_movingmode.bt.index, (boolean)pm_obs_movingmode.bt.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_BT_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_WLAN_MODE)
	{
		pm_obs_a_wlan( (pm_obs_mode_wlan_type)pm_obs_movingmode.wlan.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_WLAN_MODE;
	}

	if (pm_obs_movingmode.bit & PM_MOVING_SENSOR_MODE)
	{
		pm_obs_a_sensor( (pm_obs_mode_sensor_type)pm_obs_movingmode.sensor.index );
		pm_obs_movingmode.bit &= ~PM_MOVING_SENSOR_MODE;
	}
	...
	if (pm_obs_movingmode.bit & PM_MOVING_DTV_MODE)
	{
		pm_obs_a_dtv( pm_obs_movingmode.dtv.index, (boolean)pm_obs_movingmode.dtv.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_DTV_MODE;
	}
	
	if (pm_obs_movingmode.bit & PM_MOVING_FELICA_MODE)
	{
		pm_obs_a_felica( pm_obs_movingmode.felica.index, (boolean)pm_obs_movingmode.felica.value );
		pm_obs_movingmode.bit &= ~PM_MOVING_FELICA_MODE;
	}
...
int pm_xo_adc_read(u32 *xo_therm_adc)
{
	int rc ;
	struct rpc_req_batt_chg {
		struct rpc_request_hdr hdr;
	...
	struct rpc_rsp_batt_chg {
		struct rpc_reply_hdr hdr;
		u32 more_data;
	} rsp_batt_chg;

	memset(&rsp_batt_chg, 0, sizeof(rsp_batt_chg));

	rc = msm_rpc_call_reply(msm_batt_info.chg_ep,
			ONCRPC_CHG_XO_THERM_ADC_READ_PROC,
				&req_batt_chg, sizeof(req_batt_chg),
				&rsp_batt_chg, sizeof(rsp_batt_chg),
				msecs_to_jiffies(BATT_RPC_TIMEOUT));
	if (rc < 0) {
		pr_err("%s: ERROR. msm_rpc_call_reply failed! proc=%d rc=%d\n",
		       __func__, ONCRPC_CHG_XO_THERM_ADC_READ_PROC, rc);
		return rc;
	} else if (be32_to_cpu(rsp_batt_chg.more_data)) {
		*xo_therm_adc = be32_to_cpu(rsp_batt_chg.more_data);
	} else {
		pr_err("%s: No battery/charger data in RPC reply\n", __func__);
		return -EIO;
	}
...
EXPORT_SYMBOL(pm_xo_adc_read);
 

 
void pm_usb_ovp_notify(boolean _flag)
{
	int rc;

	struct rpc_req_usb_ovp {
		struct rpc_request_hdr hdr;
		u32 flag;
	} req_usb_ovp;

	struct rpc_rsp_usb_ovp {
		struct rpc_reply_hdr hdr;
	} rsp_usb_ovp;
	...
	if (msm_batt_info.chg_ep != NULL)	{
		...
		memset(&rsp_usb_ovp, 0, sizeof(rsp_usb_ovp));
		req_usb_ovp.flag = cpu_to_be32(_flag);

        printk(KERN_ERR "[%s] OVP flag = %d\n",__func__, _flag);

        rc = msm_rpc_call_reply(msm_batt_info.chg_ep,
				ONCRPC_CHG_USB_OVP_NOTIFY_PROC,
					&req_usb_ovp, sizeof(req_usb_ovp),
					&rsp_usb_ovp, sizeof(rsp_usb_ovp),
					msecs_to_jiffies(BATT_RPC_TIMEOUT));

		if (rc < 0) {
			pr_err("%s: FAIL: pm_usb_ovp_notify rc=%d\n", __func__, rc);
		}

	} else {
		
		ovp_pending_data = _flag;
		ovp_pending_status = TRUE;
		pr_info("%s:ovp_flag=%d: Wait until msm_batt_init()\n", __func__, (int)_flag);
	}
...
EXPORT_SYMBOL(pm_usb_ovp_notify);
 

void pm_usb_ovprecv_notify(boolean _flag)
{
	int rc;

	struct rpc_req_usb_ovprecv {
		struct rpc_request_hdr hdr;
		u32 flag;
	} req_usb_ovprecv;

	struct rpc_rsp_usb_ovprecv {
		struct rpc_reply_hdr hdr;
	} rsp_usb_ovprecv;

	memset(&rsp_usb_ovprecv, 0, sizeof(rsp_usb_ovprecv));
	req_usb_ovprecv.flag = cpu_to_be32(_flag);

    printk(KERN_ERR "[%s] OVPRECV \n",__func__ );

    rc = msm_rpc_call_reply(msm_batt_info.chg_ep,
			ONCRPC_CHG_USB_OVPRECV_NOTIFY_PROC,
				&req_usb_ovprecv, sizeof(req_usb_ovprecv),
				&rsp_usb_ovprecv, sizeof(rsp_usb_ovprecv),
				msecs_to_jiffies(BATT_RPC_TIMEOUT));

	if (rc < 0) {
		pr_err("%s: FAIL: pm_usb_ovprecv_notify rc=%d\n", __func__, rc);
	}

	return;
}
EXPORT_SYMBOL(pm_usb_ovprecv_notify);

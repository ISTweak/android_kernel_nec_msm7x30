...
static int msm_fb_notify_reboot_flg = 0;
					...
					enum led_brightness value)
{
	...
	if (value > mfd->panel_info.bl_max ){
		value = mfd->panel_info.bl_max;
	}
...
#define MSM_FB_LCD_CNT 2
static int lcd_cnt = 0;
			...
			unsigned long arg)
{
    ...
    struct msmfb_regset reg_data;


    struct msmfb_vsync_ave vsync_ave;


	int ret = 0;


    struct msmfb_vsync_parame vsync_freq;
    ...
    struct msmfb_regset msmfb_cabc_regset[10];




	struct mdp4_vg_csc_matrix vg_csc_matrix;
    ...
    case MSMFB_SET_VG_CCS_MATRIX:
      ret = copy_from_user(&vg_csc_matrix, argp, sizeof(vg_csc_matrix));
      if (ret) {
        printk(KERN_ERR
          "%s:MSMFB_SET_VG_CCS_MATRIX ioctl failed \n",__func__);
        return ret;
      }

      down(&msm_fb_ioctl_ppp_sem);
      mdp4_vg_csc_update(&vg_csc_matrix);
      up(&msm_fb_ioctl_ppp_sem);
      break;
    ...
    case MSMFB_CUSTOM_1 :
        ...
        ret = mddi_nt35560_standby_ctl(1);
    ...
    case MSMFB_CUSTOM_2 :
        ...
        ret = mddi_nt35560_standby_ctl(0);
    ...
    case MSMFB_CUSTOM_3 :
        ...
        ret = mddi_nt35560_deep_standby_ctl(1);
    ...
    case MSMFB_CUSTOM_4 :
        ...
        ret = mddi_nt35560_deep_standby_ctl(0);
    ...
    case MSMFB_CUSTOM_6 :
        ...
        mddi_nt35560_register_access_packet(reg_data.reg_addr, reg_data.set_data);
    ...
    case MSMFB_CUSTOM_7 :
        ret = mddi_nt35560_mdp_vsync_freq_check(&vsync_ave);
        if (ret)
        {
            break;
        }
        
        ret = copy_to_user(argp, &vsync_ave, sizeof(vsync_ave));
        break;

    case MSMFB_CUSTOM_8:
    {
        ret = copy_from_user(&vsync_freq, argp, sizeof(vsync_freq));
        if(ret)
        {
            break;
        }
        
        mddi_nt35560_set_vsync_freq(&vsync_freq, (void *)mfd);
        ret = 0;
        break;
    }
    ...
    case MSMFB_CUSTOM_9:
    {
        ...
        ret = mddi_nt35560_user_request_ctrl(&user_req);
        if (ret)
        {
            break;
        }
    ...
    case MSMFB_CUSTOM_10 :
        mddi_nt35560_set_standby_ctl(1);
        ret = 0;
        break;

    case MSMFB_CUSTOM_11 :
        mddi_nt35560_set_standby_ctl(0);
        ret = 0;
        break;
    ...
    case MSMFB_CUSTOM_100:
        ...
        ret = mddi_nt35560_cabc_ctl(1);
        if(ret)
            MSM_FB_ERR("%s(%d): ret=%d.\n", __func__, __LINE__, ret);
        break;

    case MSMFB_CUSTOM_101:
        
        ret = mddi_nt35560_cabc_ctl(0);
        if(ret)
            MSM_FB_ERR("%s(%d): ret=%d.\n", __func__, __LINE__, ret);
        break;


    case MSMFB_CUSTOM_102:
        
        ret = copy_from_user(&msmfb_cabc_regset, argp, sizeof(msmfb_cabc_regset));
        ...
        ret = mddi_nt35560_set_cabc(msmfb_cabc_regset);
        if(ret)
            MSM_FB_ERR("%s(%d): ret=%d.\n", __func__, __LINE__, ret);
        break;

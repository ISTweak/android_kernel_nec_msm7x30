...
static void mdp4_vg_csc_mv_update(int vp_num, uint32 *csc_mv )
{
  int i;
  for (i = 0; i < 9; i++) {
    csc_matrix_tab[i] = csc_mv[i];
  }
  mdp4_vg_csc_mv_setup(vp_num);
}

static void mdp4_vg_csc_pre_bv_update(int vp_num, uint32 *csc_pre_bv )
{
  int i;
  for (i = 0; i < 3; i++) {
    csc_pre_bv_tab[i] = csc_pre_bv[i];
  }
  mdp4_vg_csc_pre_bv_setup(vp_num);
}

static void mdp4_vg_csc_post_bv_update(int vp_num, uint32 *csc_post_bv )
{
  int i;
  for (i = 0; i < 3; i++) {
    csc_post_bv_tab[i] = csc_post_bv[i];
  }
  mdp4_vg_csc_post_bv_setup(vp_num);
}

static void mdp4_vg_csc_pre_lv_update(int vp_num, uint32 *csc_pre_lv )
{
  int i;
  for (i = 0; i < 6; i++) {
    csc_pre_lv_tab[i] = csc_pre_lv[i];
  }
  mdp4_vg_csc_pre_lv_setup(vp_num);
}

static void mdp4_vg_csc_post_lv_update(int vp_num, uint32 *csc_post_lv )
{
  int i;
  for (i = 0; i < 6; i++) {
    csc_post_lv_tab[i] = csc_post_lv[i];
  }
...
{
  mdp4_vg_csc_mv_update(vg_csc_matrix->VG_id, vg_csc_matrix->csc_mv );
  mdp4_vg_csc_pre_bv_update(vg_csc_matrix->VG_id, vg_csc_matrix->csc_pre_bv );
  mdp4_vg_csc_post_bv_update(vg_csc_matrix->VG_id, vg_csc_matrix->csc_post_bv );
  mdp4_vg_csc_pre_lv_update(vg_csc_matrix->VG_id, vg_csc_matrix->csc_pre_lv );
  mdp4_vg_csc_post_lv_update(vg_csc_matrix->VG_id, vg_csc_matrix->csc_post_lv );
}

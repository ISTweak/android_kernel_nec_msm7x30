...
void mdp4_mddi_dma_busy_wait(struct msm_fb_data_type *mfd)
{
	...
	if (need_wait) {
		...
		result = wait_for_completion_timeout(&mfd->dma->comp, msecs_to_jiffies(50));

		if(!result)
		{
			printk(KERN_INFO "mdp4_mddi_dma_busy_wait: There's no interrupt.\n");
				...
				struct mdp4_overlay_pipe *pipe)
{
    ...
    if( mddi_nt35560_displaying_chk() != TRUE )
    {
        
        mddi_nt35560_enable_display();
    }

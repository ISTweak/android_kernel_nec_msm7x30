...
static int diag_check_client(int cmd_code, int subsys_id, uint16_t subsys_cmd_code)
{
	int	i, ret;
	...
	/* Check for registered clients and forward packet to user-space */
		...
		if (g_b_mode == D_DIAG_B_MODE) {
			if (subsys_cmd_code == D_DIAG_MODE_CMD) {
				index = diag_check_client(cmd_code, subsys_id, subsys_cmd_code);
				if (index != -1) {
					driver->pkt_length = len;
					diag_update_pkt_buffer(buf);
					diag_update_sleeping_process(driver->table[index].process_id);
					return 0;
				}
			}

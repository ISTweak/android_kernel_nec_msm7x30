...
#define SYNOPEX_MODE_ENTER_2SCAN   0x04  
...
#define SYNOPEX_MODE_BUTTON    0x01 
#define SYNOPEX_MODE_ERR_TOUCH 0x02 
#define SYNOPEX_MODE_2ND_TOUCH 0x02 
#define SYNOPEX_MODE_1ST_TOUCH 0x04 

#define SYNOPEX_MODE_BUTTON_CHECK	0x07	
...
#define SYNOPEX_FWUP_RET_ERR_SRESET			0x01	
#define SYNOPEX_FWUP_RET_ERR_GPIO			0x02	
#define SYNOPEX_FWUP_RET_ERR_INIT_I2C		0x03	
#define SYNOPEX_FWUP_RET_ERR_I2C_READ_ACK	0x10	
#define SYNOPEX_FWUP_RET_ERR_I2C_READ_OTHER	0x11	
#define SYNOPEX_FWUP_RET_ERR_I2C_WRITE_ACK	0x12	
#define SYNOPEX_FWUP_RET_ERR_I2C_WRITE_OTHER 0x13	
#define SYNOPEX_FWUP_RET_ERR_FW_NO_FILE		0x20	
#define SYNOPEX_FWUP_RET_ERR_BUSY_TOUT		0x21	
#define SYNOPEX_FWUP_RET_ERR_CSUM			0x22	
#define SYNOPEX_FWUP_RET_ERR_UNINIT			0x23	
#define SYNOPEX_FWUP_RET_ERR_FW_SAME		0x99	
#define SYNOPEX_FWUP_RET_ERR_OTHER			0xFF	
...
#define	SYNOPEX_I2C_IDA_VE243				0xBF
...
#define	I2C_LOCAL_ERROR_WRITE_DATA			0xA0 ;
#define	I2C_LOCAL_ERROR_READ_DATA			0xC0 ;
...
static unsigned char i2c_local_sda_get( unsigned int n_time)
{
	...
	data = gpio_get_value(GPIO_LOCAL_SDA_PORT);		
...
static int i2c_local_write( unsigned char data )
{
	...
	for( bit_cnt = 8 ; bit_cnt > 0 ; bit_cnt-- ){
		...
		i2c_local_scl_set( ON,  WAIT_1_2_CYCLE );	
		
	}
	i2c_local_scl_set( OFF, 0 );		
	
	
	i2c_local_sda_read_mode_set();
	
	i2c_local_scl_set( OFF, WAIT_1_2_CYCLE );		
...
static int synopex_jump_reset( void )
{
	...
	cmd_string[0]=SYNOPEX_I2C_IDA_VE243;
	result=synopex_i2c_write_cmd(cmd_string,1);
	if(result!=0) {
		printk(KERN_ERR "[synopex]%s write data failed: %x\n", __func__, result);
		return result;
...
static int __devinit synopex_probe(struct spi_device *spi)
{
	...
	INIT_WORK(&work2, spi_touch_2scan_enter );	
	INIT_WORK(&work3, spi_touch_2scan_exit );	
...
#define VE321_PROC_TOUCH_ANDROID_LBS       "nec_touch_android_lbs"
#define VE321_PROC_TOUCH_VZW_LBS           "nec_touch_vzw_lbs"
#define VE321_WRITEBUF_SIZE 64
...
static int nec_touch_android_lbs_proc_write(struct file* filp, const char* buffer, unsigned long count, void* data )
{
	...
	if ( count > VE321_WRITEBUF_SIZE ) {
		copy_len = VE321_WRITEBUF_SIZE;
	}
	else {
 		copy_len = count;
	}
...
static int __init nec_proc_init(void)
{
	...
	dirp = (struct proc_dir_entry *)
		create_proc_entry(VE321_PROC_TOUCH_ANDROID_LBS, 0222, 0);
	if (dirp == 0) {
		return(-EINVAL);
	}
	...
	dirp = (struct proc_dir_entry *)
		create_proc_entry(VE321_PROC_TOUCH_VZW_LBS, 0222, 0);
	if (dirp == 0) {
		return(-EINVAL);
	}
...
{
	remove_proc_entry(VE321_PROC_TOUCH_ANDROID_LBS, NULL);
	remove_proc_entry(VE321_PROC_TOUCH_VZW_LBS, NULL);
}

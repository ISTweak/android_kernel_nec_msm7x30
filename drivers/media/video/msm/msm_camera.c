...
static void msm_frame_buffer_free(struct msm_vfe_resp *, void *);
...
static int msm_get_stats(struct msm_sync *sync, void __user *arg)
{
	...
	case MSM_CAM_Q_TIMER:
		
		se.resptype = MSM_CAM_RESP_TIMER;
		break;
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_SENSOR_IO_CFG:
		...
		if(pmsm->sync->sctrl.s_config2){
			
			rc = pmsm->sync->sctrl.s_config2((void *)pmsm->sync, argp);
		} else {
...
static long msm_ioctl_init(struct file *filep, unsigned int cmd,
	unsigned long arg)
{
	uint8_t work_flg;

	switch (cmd) {
	case MSM_CAM_IOCTL_CAMERA_ENABLE:
		CAMCMP_LOG_DBG("cmd MSM_CAM_IOCTL_CAMERA_ENABLE\n");
		
		if (copy_from_user(&work_flg, (void __user *)arg, sizeof(uint8_t))) {
			ERR_COPY_FROM_USER();
			CAMCMP_LOG_ERR("copy error\n");
			return -EFAULT;
		}

		
		camera_enable_flg = work_flg;
		CAMCMP_LOG_DBG("camera_enable_flg=%d\n", camera_enable_flg);
		break;

	default:
		CAMCMP_LOG_ERR("cmd error: cmd=%d\n", cmd);
		return -EFAULT;
	}
...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			...
			pm_obs_a_camera(PM_OBS_CAMERA_MODE, 0);
...
static int msm_release_init(struct inode *node, struct file *filep)
{
	struct msm_cam_device *pmsm = filep->private_data;
	CAMCMP_LOG_DBG("release %s\n", filep->f_path.dentry->d_name.name);
	atomic_set(&pmsm->opened, 0);
	return 0;
}
...
{
	struct msm_sync *sync = (struct msm_sync *)data;
	struct msm_queue_cmd *qcmd = kmalloc(sizeof(struct msm_queue_cmd), GFP_KERNEL);

	CDBG("msm_af_start_poll start \n");

	if ((!qcmd) || (!sync)){
		CDBG("msm_af_start_poll input parm error!! qcmd = %p, sync = %p\n",qcmd,sync);
	...
	qcmd->type = MSM_CAM_Q_TIMER;
	msm_enqueue(&sync->event_q, &qcmd->list_config);
	CDBG("msm_af_start_poll end \n");
}
EXPORT_SYMBOL(msm_af_start_poll);
		...
		gfp_t gfp)
{
	...
	case VFE_MSG_OUTPUT_P:
		...
		if (sync->frame_q.len <= 100 && sync->event_q.len <= 100) {
			...
			msm_frame_buffer_free(vdata, sync);
			...
			int is_controlnode)
{
	...
	if (!sync->core_powered_on && !is_controlnode) {
		...
		if (sync->vfefn.vfe_init) {
			...
			pm_obs_a_camera(PM_OBS_CAMERA_MODE, 1);
			...
			int once, int is_controlnode)
{
	...
	if ( camera_enable_flg == 0 ) {
		CAMCMP_LOG_ERR("camera not support\n");
		return -ENODEV;
	}
...
static int msm_open_init(struct inode *inode, struct file *filep)
{
	int rc;
	struct msm_cam_device *pmsm =
		container_of(inode->i_cdev, struct msm_cam_device, cdev);

	CAMCMP_LOG_DBG("open %s\n", filep->f_path.dentry->d_name.name);

	atomic_cmpxchg(&pmsm->opened, 0, 1);
...
static const struct file_operations msm_fops_init = {
	.owner = THIS_MODULE,
	.open = msm_open_init,
	.unlocked_ioctl = msm_ioctl_init,
	.release = msm_release_init,
};
...
static void msm_frame_buffer_free(struct msm_vfe_resp *VE365_resp, void *syncdata)
{
    struct msm_sync *sync = (struct msm_sync *)syncdata;
	struct msm_pmem_info pinfo;
	struct msm_frame frame;
	...
	if(msm_pmem_frame_ptov_lookup(sync, VE365_resp->phy.y_phy, VE365_resp->phy.cbcr_phy, &pinfo, 1) >= 0)
	{
        
        if(VE365_resp->type == VFE_MSG_OUTPUT_V) frame.path = OUTPUT_TYPE_V;
        else                                    frame.path = OUTPUT_TYPE_P;
        frame.buffer    = (unsigned long)pinfo.vaddr;
        frame.y_off     = pinfo.y_off;
        frame.cbcr_off  = pinfo.cbcr_off;
        frame.fd        = pinfo.fd;
        
        if(__msm_put_frame_buf(sync, &frame) < 0)
        {
    		pr_err("%s: __msm_put_frame_buf failed\n", __func__);
        }
    }
		...
		int node)
{
	...
	rc = msm_setup_cdev(pmsm + 3, node,
		...
		"init", &msm_fops_init);
	if (rc < 0) {
		pr_err("%s: error creating init node: %d\n", __func__, rc);
		msm_tear_down_cdev(pmsm,
			MKDEV(MAJOR(msm_devno), dev_num));
		msm_tear_down_cdev(pmsm + 1,
			MKDEV(MAJOR(msm_devno), dev_num + 1));

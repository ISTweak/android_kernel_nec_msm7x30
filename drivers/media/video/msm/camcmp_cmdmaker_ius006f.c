...
#define CAMCMP_AF_CAF_PARAM_WOBBLE_STEP		68		
#define CAMCMP_AF_CAF_CLIMB_STEP			69		
#define CAMCMP_AF_CAF_WOBBLE_MAX_MOVE		70		
#define CAMCMP_AF_SEARCH_OFFSET				71		
#define CAMCMP_AF_DRV_AMOUNT_TONEAR_F		72		
#define CAMCMP_AF_DRV_AMOUNT_TOFAR_F		73		
#define CAMCMP_AF_DRV_AMOUNT_TONEAR_S		74		
#define CAMCMP_AF_DRV_AMOUNT_TOFAR_S		75		
#define CAMCMP_AF_AREA_LOW_TYPE1			76		
#define CAMCMP_AF_AREA_HIGH_TYPE1			77		
#define CAMCMP_AF_LENSPOS_ON_AFNG			79		
#define CAMCMP_AF_AREA_LOW_TYPE2			80		
#define CAMCMP_AF_AREA_HIGH_TYPE2			81		
#define CAMCMP_AF_AREA_LOW_TYPE3			82		
#define CAMCMP_AF_AREA_HIGH_TYPE3			83		
...
static int32_t camcmp_ius006f_write_single(unsigned short addr, unsigned int val, unsigned short size)
{
	...
	if (addr >= 0x0000 && addr < 0x0400 && size == 2) {
		
		buf[2] = (val & 0x0000FF00) >> 8;
		buf[3] = (val & 0x000000FF);
	
	} else if (addr >= 0x0000 && addr < 0x0400 && size == 4) {
	...
	if (rc < 0) {
		...
		ALARM_LOG("Event:0x%02X Info:0x%02X \n", ALARM_EVENT_CAMERA, ALARM_INFO_CAMERA);
...
static int32_t camcmp_ius006f_read(unsigned short addr, unsigned short size, uint32_t* p_rev)    
{
	int32_t rc = 0;
...
static int32_t camcmp_ius006f_set_blank(camcmp_data_type blank_info, void* p_in_data)
{
	...
	for (i = 0; i < blank_info.size; i++) {
		...
		case CAMCMP_D_LEN_BYTE:
			p_char = (unsigned char*)p_blank->p_copy;
			if (p_in_data == NULL) {
				*p_char = (unsigned char)p_blank->org_val;
			} else {
				*p_char = (unsigned char)val;
			}
			break;
...
static int32_t camcmp_ius006f_send_cmd(camcmp_data_type send_cmd, void* p_rev, uint32_t* p_rev_num, uint32_t max_num)
{
	...
	for (i = 0; i < send_cmd.size; i++) {
		...
		case CAMCMP_REG_READ:
			...
			p_rev_data = ((uint32_t*)p_rev) + *p_rev_num;   
...
static int32_t camcmp_ius006f_correct_cmd(void* p_val, uint32_t num)
{
	...
	dac_overinf = ((g_p_camcmp_ius006f_af->fstep * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 

	
	opt_inf_pos = ((dac1m - ((dis_bet_pos * (dac10cm - dac1m) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_100)) / CAMCMP_CALC_1000)
	                - g_p_camcmp_ius006f_af->dacoffset) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_10)) / CAMCMP_CALC_100; 

	
	macro_pos = ((dac10cm - g_p_camcmp_ius006f_af->dacoffset) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_10)) / CAMCMP_CALC_100; 

	
	stand_pos = ((dac1m - g_p_camcmp_ius006f_af->dacoffset) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_10)) / CAMCMP_CALC_100; 

	
	af_drv_search_f = ((g_p_camcmp_ius006f_af->fstep * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 

	
	
	af_drv_search_s = ((g_p_camcmp_ius006f_af->sstep * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 
	...
	high_search_area = (((macro_pos * CAMCMP_CALC_100) + g_p_camcmp_ius006f_af->dacovermacro) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_10)) / CAMCMP_CALC_100; 
	...
	if( p_reg[g_camcmp_af_search_offset].waddr == 0x4844 ) {
		p_reg[g_camcmp_af_search_offset].lwdata = ((g_p_camcmp_ius006f_af->frange * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 
	} else {
	...
	if( p_reg[g_camcmp_af_caf_param_wobble_step].waddr == 0x4822 ) {
		p_reg[g_camcmp_af_caf_param_wobble_step].lwdata = ((g_p_camcmp_ius006f_af->wstep * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 
	} else {
	...
	if( p_reg[g_camcmp_af_caf_climb_step].waddr == 0x4824 ) {
		p_reg[g_camcmp_af_caf_climb_step].lwdata = ((g_p_camcmp_ius006f_af->VE363 * vcm_slope / 100) + (CAMCMP_CALC_ROUND_OFF * CAMCMP_CALC_1000)) / CAMCMP_CALC_10000; 
	} else {
	...
	if( p_reg[g_camcmp_af_caf_wobble_max_move].waddr == 0x4838 ) {
		p_reg[g_camcmp_af_caf_wobble_max_move].lwdata = p_reg[g_camcmp_af_caf_param_wobble_step].lwdata * 2; 
	} else {

...
const struct msm_gpio deep_sleep_config_data[] ={ { GPIO_CFG(97, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "WLAN_SHDN"}};
int libra_sdio_deep_sleep_config(void)
{
   int ret = 0;

   ret = msm_gpios_request_enable(deep_sleep_config_data, ARRAY_SIZE(deep_sleep_config_data));

   return(ret);
}
EXPORT_SYMBOL(libra_sdio_deep_sleep_config);

void libra_sdio_deep_sleep_deconfig(void)
{

   msm_gpios_disable_free(deep_sleep_config_data, ARRAY_SIZE(deep_sleep_config_data));

}
EXPORT_SYMBOL(libra_sdio_deep_sleep_deconfig);

void libra_sdio_deep_sleep_in(void)
{
   gpio_set_value(97,0);
}
EXPORT_SYMBOL(libra_sdio_deep_sleep_in);

void libra_sdio_deep_sleep_out(void)
{
   gpio_set_value(97,1);
}
EXPORT_SYMBOL(libra_sdio_deep_sleep_out);

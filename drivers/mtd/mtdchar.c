...
static ssize_t mtd_read_marm(struct file *file, char __user *buf, size_t count,loff_t *ppos);
static ssize_t mtd_write_marm(struct file *file, const char __user *buf, size_t count,loff_t *ppos);
...
static ssize_t mtd_read_marm(struct file *file, char __user *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
        ...
        ret = mtd->read_marm(mtd, *ppos, len, &retlen, kbuf);
...
static ssize_t mtd_write_marm(struct file *file, const char __user *buf, size_t count,loff_t *ppos)
{
	...
	while (count) {
		...
		ret = (*(mtd->write_marm))(mtd, *ppos, len, &retlen, kbuf);
...
static struct smem *smem = (void *)(MSM_SHARED_RAM_BASE);
...
unsigned mtd_smem_read_alloc_entry(smem_mem_type_t type, void *buf, int len)
{
    ...
    src = (unsigned)MSM_SHARED_RAM_BASE + readl(&ainfo->offset);
...
static int mtd_ioctl(struct file *file, u_int cmd, u_long arg)
{
    ...
    case MEMREADPART:
    {
        struct smem_ptable  smem_ptable;

        if ( mtd_smem_read_alloc_entry( SMEM_AARM_PARTITION_TABLE, &smem_ptable, sizeof(struct smem_ptable) ) )
        {
            return -EFAULT;
        }
        if ( copy_to_user( argp, &smem_ptable, sizeof(struct smem_ptable) ) )
        {
            return -EFAULT;
        }
        break;
    ...
    case MEMREADMARM:
    {
        struct mtd_marm_buf marm_buf;

        
        if (copy_from_user( &marm_buf, argp, sizeof( struct mtd_marm_buf )))
            return -EFAULT;
        
        
        ret = mtd_read_marm( file, marm_buf.buf, marm_buf.count, &marm_buf.ppos);
        break;
    }
    ...
    case MEMWRITEMARM:
    {
        ...
        ret = mtd_write_marm( file, marm_buf.buf, marm_buf.count, &marm_buf.ppos);
        break;
    }

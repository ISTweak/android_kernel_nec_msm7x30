...
extern struct nand_lock_info    msm_nand_lock_info[NAND_LOCK_PARTITION_NUM];
extern unsigned int             msm_nand_lock_info_num;
...
static int msm_nand_read_oob_dualnandc_marm(struct mtd_info *mtd, loff_t from,
			struct mtd_oob_ops *ops)
{
	...
	if (chip->CFG1_marm & CFG1_WIDE_FLASH)
		oob_col >>= 1;
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
			...
			if (start_sector == (cwperpage - 1)) {
				dma_buffer->data.cfg0 = (chip->CFG0_marm &
							~(7U << 6));
			} else {
				dma_buffer->data.cfg0 = (chip->CFG0_marm &
				~(7U << 6))
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (n == start_sector) {
				...
				dma_buffer->data.ecccfg = chip->ecc_buf_cfg_marm;
				cmd->cmd = 0;
				cmd->src = msm_virt_to_dma(chip,
...
static int
msm_nand_write_oob_dualnandc_marm(struct mtd_info *mtd, loff_t to,
				struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	struct {
    ...
    for( cnt = 0; cnt < msm_nand_lock_info_num; cnt++ )
    {
        if ( msm_nand_lock_info[cnt].start_addr <= to
        &&   msm_nand_lock_info[cnt].end_addr   >  to )
        {
            pr_err( "nand lock write dual error 0x%012llx : %s\n",
                    (unsigned long long)to,
                    &msm_nand_lock_info[cnt].name[0] );
            return -EROFS;
        }
    }
	...
	while (page_count-- > 0) {
		...
		if (ops->mode != MTD_OOB_RAW) {
			dma_buffer->data.cfg0 = ((chip->CFG0_marm & ~(7U << 6))
				| (1 << 4)) | ((((cwperpage >> 1)-1)) << 6);
			dma_buffer->data.cfg1 = chip->CFG1_marm;
		} else {
			dma_buffer->data.cfg0 = ((MSM_NAND_CFG0_RAW &
...
static int
msm_nand_read_marm(struct mtd_info *mtd, loff_t from, size_t len,
	      size_t *retlen, u_char *buf)
{
	...
	ops.datbuf = buf;
	ops.oobbuf = NULL;
	ret = msm_nand_read_oob_dualnandc_marm(mtd, from, &ops);
	*retlen = ops.retlen;
	return ret;
}

static int msm_nand_write_marm(struct mtd_info *mtd, loff_t to, size_t len,
			  size_t *retlen, const u_char *buf)
{
	...
	ops.oobbuf = NULL;
	ret =  msm_nand_write_oob_dualnandc_marm(mtd, to, &ops);
	*retlen = ops.retlen;
	return ret;
}
...
msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
    ...
    for( cnt = 0; cnt < msm_nand_lock_info_num; cnt++ )
    {
        ...
        &&   msm_nand_lock_info[cnt].end_addr   >  to )
        {
            pr_err( "nand lock write error 0x%012llx : %s\n",
                    (unsigned long long)to,
                    &msm_nand_lock_info[cnt].name[0] );
            return -EROFS;
        }
...
err_dma_map_oobbuf_failed:
	...
	if (err)
	{
		pr_err("[T][ARM]Event:0x21 Info:0x01\n");
		pr_err("msm_nand_write_oob_dualnandc %llx %x %x failed %d\n",
		       to, ops->len, ops->ooblen, err);
	}
...
msm_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
    ...
    for( cnt = 0; cnt < msm_nand_lock_info_num; cnt++ )
    {
        if ( msm_nand_lock_info[cnt].start_addr <= instr->addr
        &&   msm_nand_lock_info[cnt].end_addr   >  instr->addr )
        {
            pr_err( "nand lock erase error 0x%012llx : %s\n",
	...
	if (err) {
		...
		pr_err("[T][ARM]Event:0x21 Info:0x02\n");
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
    ...
    chip->CFG0_marm        = (((mtd_writesize >> 9) - 1) <<  6)  
        |                    (512 <<  9)                         
        |                    (10 << 19)                          
        |                    (5 << 27)                           
        |                    (0 << 30)                           
        |                    (1 << 31)                           
        |                    ((wide_bus) ? (4 << 23) : (5 << 23));
    chip->CFG1_marm        = chip->CFG1;
    chip->ecc_buf_cfg_marm = 0x1FF;
    ...
    mtd->read_marm = msm_nand_read_marm;
    mtd->write_marm = msm_nand_write_marm;

	...
	list_for_each_entry(f, &config->functions, list) {
		...
		while ((descriptor = *descriptors++) != NULL) {
			...
			if (intf->bDescriptorType == USB_DT_INTERFACE) {
				...
				cdc_header = (struct usb_cdc_header_desc *)dest;
					if (cdc_header->bDescriptorSubType == USB_CDC_CALL_MANAGEMENT_TYPE) {
						struct usb_cdc_call_mgmt_descriptor *cdc_call_mgmt;
						cdc_call_mgmt = (struct usb_cdc_call_mgmt_descriptor *)dest;
						cdc_call_mgmt->bDataInterface = interfaceCount;
					}
					else if (cdc_header->bDescriptorSubType == USB_CDC_UNION_TYPE) {
						struct usb_cdc_union_desc *cdc_union;
						cdc_union = (struct usb_cdc_union_desc *)dest;
						cdc_union->bMasterInterface0 = interfaceCount - 1;
						cdc_union->bSlaveInterface0 = interfaceCount;
					}
			}

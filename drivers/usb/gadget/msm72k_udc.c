...
#define PMIC_GPIO_AUD_CONN_ID        102
#define PMIC_GPIO_HIGH_VALUE    1
#define PMIC_GPIO_LOW_VALUE     0

extern bool g_pmic_usb_audio_gpio_stat;
extern boolean usb_vbus_flg;
...
extern bool g_usb_audio_flg;    


 
extern bool g_fsa928x_power_supply_flg; 



extern usb_headset_info_type g_usb_headset_info;    


extern bool g_fsa928x_sdp_flg;                      
...
enum {
	EN_USB_DISCONNECTED = 0,
	EN_USB_CONNECTED,
	EN_USB_INVALID
};
int	msm72k_udc_usb_state = EN_USB_DISCONNECTED;
static bool	msm72k_udc_connected = false;
...
int usb_fs_mode_configured(void);
int usb_hs_mode_configured(void);
...
int usb_diag_dev_file_ioctl(struct inode*, struct file*,
...
int usb_diag_read_connection(void);
int usb_diag_write_regulation(unsigned char* regulate_cmd);
int usb_diag_read_register(unsigned char* reg);
int usb_diag_write_register(unsigned char* ioctl_cmd);
...
int usb_diag_set_path_vbus_in(usb_diag_vbus_in_path_enum usb_diag_vbus_in_path, unsigned char* manual_sw1_value);    

static struct delayed_work usb_diag_polling_work;           
bool g_diag_usb_work_polling_flag = false;                  


int usb_set_detect_earphone_switch(unsigned char* detect_earphone_switch);
extern bool g_usb_earphone_switch_detect_flg;
extern void fsa928x_start_adc_polling( void );
extern void fsa928x_stop_adc_polling( void );
extern struct mutex g_fsa928x_usb_main_lock;
...
extern unsigned char g_usb_mic_dec_waittime;


extern void android_callback(struct usb_comm_response *response, void * puser);
...
struct usb_info *usbinfo_st = NULL;



struct cdev       usb_diag_cdev;   

struct class      *usb_diag_class = NULL;   
...
static inline enum chg_type usb_get_chg_type(struct usb_info *ui)
{
    ...
    {
            if(g_fsa928x_sdp_flg == true)
            {
                printk(KERN_INFO "[%s] <USB_CHG_TYPE__SDP>\n", __func__);
                return USB_CHG_TYPE__SDP;
            }
            else
            {
                printk(KERN_INFO "[%s] <USB_CHG_TYPE__WALLCHARGER>\n", __func__);
                return USB_CHG_TYPE__WALLCHARGER;
            }
        }
...
static void usb_chg_detect(struct work_struct *w)
{
    ...
    printk(KERN_INFO "[%s] START g_fsa928x_power_supply_flg=%d\n", __func__, g_fsa928x_power_supply_flg);     

	spin_lock_irqsave(&ui->lock, flags);
	if (ui->usb_state == USB_STATE_NOTATTACHED) {
        printk(KERN_INFO "[%s] usb_state=USB_STATE_NOTATTACHED\n", __func__);   
		spin_unlock_irqrestore(&ui->lock, flags);
		return;
	}
	...
	if (maxpower > 0){
        ...
        printk(KERN_INFO "[%s] otg_set_power(%d)called\n", __func__ ,maxpower);
	}
	...
	printk(KERN_DEBUG "%s: msm72k_udc_usb_state=%d, usb_get_chg_type=%d, g_usb_audio_flg=%d\n",
		__func__, msm72k_udc_usb_state, temp, g_usb_audio_flg);

	if (temp != USB_CHG_TYPE__SDP || g_usb_audio_flg == true) {
		...
		printk(KERN_DEBUG "%s: Don't show Ask on plug\n", __func__);
		...
		if (msm72k_udc_usb_state == EN_USB_CONNECTED) {
			printk(KERN_DEBUG "%s: USB connected\n", __func__);
			ret = comm_usb_request_connect(usb_connect_boot_flag);
			printk(KERN_DEBUG "%s: comm_usb_request_connect ret=%d\n",
				__func__, ret);
			if (ret == -EAGAIN) {
				msleep(10);
				ret = comm_usb_request_connect(usb_connect_boot_flag);
	...
	if(g_usb_audio_flg == true && 
       g_fsa928x_power_supply_flg == true){
		...
		printk(KERN_ERR"[USB-Audio]otg_set_power(100mA)");
		otg_set_power(ui->xceiv, 100);
	}
	else
	{
        printk(KERN_INFO"%s:g_usb_audio_flg = %d, g_fsa928x_power_supply_flg = %d", __func__, g_usb_audio_flg, g_fsa928x_power_supply_flg);
    }
...
static void handle_setup(struct usb_info *ui)
{
	...
	if ((ctl.bRequestType & (USB_DIR_IN | USB_TYPE_MASK)) ==
		...
		if (ctl.bRequest == USB_REQ_GET_DESCRIPTOR) {
			dev_info(&ui->pdev->dev, "USB_REQ_GET_DESCRIPTOR 0x%x\n", ctl.wValue);
			if (ctl.wValue == 0x2300)
				goto stall;
   		}
...
static void usb_do_work_check_vbus_ini(struct usb_info *ui)
{
	unsigned long iflags;

	spin_lock_irqsave(&ui->lock, iflags);
	printk("%s: usb_connect_boot_flag=%d\n", __func__, usb_connect_boot_flag);
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
                ...
                printk(KERN_INFO "[%s] otg_set_power(0)called\n", __func__ );
...
static int msm72k_udc_vbus_session(struct usb_gadget *_gadget, int is_active)
{
	...
	printk(KERN_ERR "msm72k_udc_vbus_session is_active = %d\n", is_active);
	if (!is_active) {


		msm72k_udc_usb_state = EN_USB_DISCONNECTED;

		if (msm72k_udc_connected) {

			if (comm_usb_is_connect_polling() == 0) {
        		printk(KERN_ERR "%s: call comm_usb_send_request disconnect\n", __func__);
			    ret = comm_usb_request_disconnect();
	       		printk(KERN_ERR "%s: return comm_usb_send_request disconnect (%d)\n", __func__, ret);
			}
		...
		msm72k_udc_connected = false;

	}
	else {

		msm72k_udc_usb_state = EN_USB_CONNECTED;
...
static int msm72k_pullup_internal(struct usb_gadget *_gadget, int is_active)
{
  ...
  int rc = 0;
	...
	if (is_active) {
		...
		if((g_usb_audio_flg == 0) && (usb_vbus_flg == true)){




		    
		    rc = msm_chg_usb_get_usb_audio(&usb_acc);
			...
			if ( usb_acc == true ) {
				...
				printk(KERN_ERR"[usb_chg_detect] connected USB_AUDIO");
				
				if ( g_pmic_usb_audio_gpio_stat != PMIC_GPIO_HIGH_VALUE )
				{
					
					gpio_set_value( PMIC_GPIO_AUD_CONN_ID, PMIC_GPIO_HIGH_VALUE );
					
					g_pmic_usb_audio_gpio_stat = PMIC_GPIO_HIGH_VALUE;
					printk(KERN_ERR"[usb_chg_detect] USB_AUD_SW set High Value");
				}
				
				hs_report_headphone_usbaudio(USBAUDIO,true);
...
int usb_ioctl_init( )
{
    ...
    if(cdev_ret < 0){
        ...
        unregister_chrdev_region( devno, DEVICE_COUNTS );
    ...
    if (IS_ERR( usb_diag_class_dev )) {
        printk(KERN_ERR "[msm72k_udc]device_create fail!\n");
        return -1;
    }
...
{
    int ret = 1;          
    struct usb_info *u_inf = usbinfo_st;
    if ( u_inf != NULL )  
	{
        if ((u_inf->usb_state == USB_STATE_CONFIGURED)
         && (u_inf->gadget.speed == USB_SPEED_FULL))
		{
            ret = 0;
		}
        else 
...
EXPORT_SYMBOL_GPL(usb_fs_mode_configured);
...
int usb_hs_mode_configured(void)
{
	...
	{
        if ((u_inf->usb_state == USB_STATE_CONFIGURED)
         && (u_inf->gadget.speed == USB_SPEED_HIGH))
		{
            ret = 0;
		}
        else 
...
EXPORT_SYMBOL_GPL(usb_hs_mode_configured);
...
int usb_diag_read_connection(void)
{
    ...
    unsigned char venderID = 0xff;
    unsigned char reg = FSA928X_DEVICE_ID_ADDRESS;

    ret = fsa928x_i2c_read(reg, &venderID);
    
    if ( ret == FSA928X_I2C_TRANSFER_NG )             
    {
        return ( ret );
    }
    ...
    switch (venderID)
    {
        case FSA9280A_DEVICEID_VALUE:
        case FSA9281_DEVICEID_VALUE:
            reg = FSA9280_ADC_ADDRESS;
            break;
        case FSA9285_DEVICEID_VALUE:
            reg = FSA9285_ADC_ADDRESS;
            break;
        default:
            return ( FSA928X_I2C_TRANSFER_NG );
    }

    ret = fsa928x_i2c_read(reg,&data);          
...
int usb_diag_write_regulation(unsigned char* regulate_cmd)
{
    ...
    switch (venderID)
    {
        case FSA9280A_DEVICEID_VALUE:
        case FSA9281_DEVICEID_VALUE:
            reg = FSA9280_CONTROL_ADDRESS;
            break;
        case FSA9285_DEVICEID_VALUE:
            reg = FSA9285_CONTROL_ADDRESS;
            break;
        default:
            return ( FSA928X_I2C_TRANSFER_NG );
    }
    ...
    if ( (*regulate_cmd == USB_DIAG_REGULATE_SWITCH_ENABLE) ||
         (*regulate_cmd == USB_DIAG_REGULATE_SWITCH_DISABLE))
    {
        ...
        else    
        {
            ...
            switch (venderID)
            {
                ...
                case FSA9281_DEVICEID_VALUE:
                    switch (*regulate_cmd){
                        case USB_DIAG_REGULATE_SWITCH_ENABLE:     
                            
                            
                            
                            ret = fsa928x_i2c_write(FSA9280_MANUALSW1_ADDRESS, 0x27);
                            data = data & FSA9280_REGULATE_SWITCH_ENABLE_MASKB;
                            break;
                        case USB_DIAG_REGULATE_SWITCH_DISABLE:    
                            
                            data = data | FSA9280_REGULATE_SWITCH_DISABLE_MASKB;
                            break;
                    }
                    break;
                case FSA9285_DEVICEID_VALUE:
                    switch (*regulate_cmd){
                        case USB_DIAG_REGULATE_SWITCH_ENABLE:     
                            ...
                            data = data & FSA9285_REGULATE_SWITCH_ENABLE_MASKB;
                            break;
                        case USB_DIAG_REGULATE_SWITCH_DISABLE:    
                            
                            data = data | FSA9285_REGULATE_SWITCH_DISABLE_MASKB;
                            break;
                    }
                    break;
                default:
            ...
            ret = fsa928x_i2c_write(reg, data);
        }
    }
    else    
    {
        printk(KERN_ERR "FSA928x usb_diag_write_regulation: cmd error\n");
        return (FSA928X_I2C_TRANSFER_NG);
    }
    
    return (FSA928X_I2C_TRANSFER_OK);
}
...
int usb_diag_read_register(unsigned char* reg)
{
    int ret = FSA928X_I2C_TRANSFER_NG;                  
    unsigned char data = 0xff;                          
    unsigned char read_reg = FSA928X_DEVICE_ID_ADDRESS;
    unsigned char venderID = 0xff;
    ret = fsa928x_i2c_read(read_reg, &venderID);
    ...
    switch(venderID)
    {
        ...
        case FSA9281_DEVICEID_VALUE:
            switch(*reg)
            {
                case FSA928X_DEVICE_ID_ADDRESS:
                    read_reg = FSA928X_DEVICE_ID_ADDRESS;
                    break;
                case FSA9280_CONTROL_ADDRESS:
                    read_reg = FSA9280_CONTROL_ADDRESS;
                    break;
                case FSA9280_INTERRUPT1_ADDRESS:
                    read_reg = FSA9280_INTERRUPT1_ADDRESS;
                    break;
                case FSA9280_INTERRUPT2_ADDRESS:
                    read_reg = FSA9280_INTERRUPT2_ADDRESS;
                    break;
                case FSA9280_INTERRUPT_MASK1_ADDRESS:
                    read_reg = FSA9280_INTERRUPT_MASK1_ADDRESS;
                    break;
                case FSA9280_INTERRUPT_MASK2_ADDRESS:
                    read_reg = FSA9280_INTERRUPT_MASK2_ADDRESS;
                    break;
                case FSA9280_ADC_ADDRESS:
                    read_reg = FSA9280_ADC_ADDRESS;
                    break;
                case FSA9280_TIMING_SET1_ADDRESS:
                    read_reg = FSA9280_TIMING_SET1_ADDRESS;
                    break;
                case FSA9280_TIMING_SET2_ADDERSS:
                    read_reg = FSA9280_TIMING_SET2_ADDERSS;
                    break;
                case FSA9280_DEVICE_TYPE1_ADDRESS:
                    read_reg = FSA9280_DEVICE_TYPE1_ADDRESS;
                    break;
                case FSA9280_DEVICE_TYPE2_ADDRESS:
                    read_reg = FSA9280_DEVICE_TYPE2_ADDRESS;
                    break;
                case FSA9280_BUTTON1_ADDRESS:
                    read_reg = FSA9280_BUTTON1_ADDRESS;
                    break;
                case FSA9280_BUTTON2_ADDRESS:
                    read_reg = FSA9280_BUTTON2_ADDRESS;
                    break;
                case FSA9280_CAR_KIT_STATUS_ADDRESS:
                    read_reg = FSA9280_CAR_KIT_STATUS_ADDRESS;
                    break;
                case FSA9280_MANUALSW1_ADDRESS:
                    read_reg = FSA9280_MANUALSW1_ADDRESS;
                    break;
                case FSA9280_MANUALSW2_ADDRESS:
                    read_reg = FSA9280_MANUALSW2_ADDRESS;
                    break;
                case FSA9280_RESET_ADDRESS:
                    read_reg = FSA9280_RESET_ADDRESS;
                    break;
                case FSA9280_VBUS_VALID_STATUS_ADDRESS:
                    read_reg = FSA9280_VBUS_VALID_STATUS_ADDRESS;
                    break;
                default:
                    return ( FSA928X_I2C_TRANSFER_NG );
                    break;
        ...
        case FSA9285_DEVICEID_VALUE:
            switch(*reg)
            {
                case FSA928X_DEVICE_ID_ADDRESS:
                    read_reg = FSA928X_DEVICE_ID_ADDRESS;
                    break;
                case FSA9285_CONTROL_ADDRESS:
                    read_reg = FSA9285_CONTROL_ADDRESS;
                    break;
                case FSA9285_INTERRUPT_ADDRESS:
                    read_reg = FSA9285_INTERRUPT_ADDRESS;
                    break;
                case FSA9285_INTERRUPT_MASK_ADDRESS:
                    read_reg = FSA9285_INTERRUPT_MASK_ADDRESS;
                    break;
                case FSA9285_ADC_ADDRESS:
                    read_reg = FSA9285_ADC_ADDRESS;
                    break;
                case FSA9285_TIMING_SET_ADDRESS:
                    read_reg = FSA9285_TIMING_SET_ADDRESS;
                    break;
                case FSA9285_DEVICE_TYPE_ADDRESS:
                    read_reg = FSA9285_DEVICE_TYPE_ADDRESS;
                    break;
                case FSA9285_DAC_SAR_ADDRESS:
                    read_reg = FSA9285_DAC_SAR_ADDRESS;
                    break;
                case FSA9285_MANUAL_CHG_CTRL_ADDRESS:
                    read_reg = FSA9285_MANUAL_CHG_CTRL_ADDRESS;
                    break;
                case FSA9285_MANUAL_SWITCH_ADDRESS:
                    read_reg = FSA9285_MANUAL_SWITCH_ADDRESS;
                    break;
                case FSA9285_STATUS_ADDERSS:
                    read_reg = FSA9285_STATUS_ADDERSS;
                    break;
                default:
                    return ( FSA928X_I2C_TRANSFER_NG );
                    break;
    ...
    ret = fsa928x_i2c_read(read_reg,&data);                      
...
int usb_diag_write_register(unsigned char* ioctl_cmd)
{
    ...
    ret = fsa928x_i2c_write(read_reg,value);                      
...
int usb_diag_start_earphone_polling(void)
{
    ...
    schedule_delayed_work(&usb_diag_polling_work, EARPHONE_POLLING_INTERVAL_TIME);
...
{
    int ret = -1;                  
    unsigned char vbus_state_value = 0xff;
    unsigned char vbus_state_reg = 0xff;
    unsigned char read_manual_sw1_value = 0xff;
    unsigned char write_manual_sw1_value = 0xff;
    unsigned char manual_sw1_reg = 0xff;
    unsigned char venderID = 0xff;
    ...
    ret = fsa928x_i2c_read(FSA928X_DEVICE_ID_ADDRESS , &venderID);
    
    if ( ret == FSA928X_I2C_TRANSFER_NG)             
    {
        g_diag_usb_work_polling_flag = false;
        usb_diag_start_earphone_polling();
        return;
    }
    ...
    switch (venderID)
    {
        case FSA9280A_DEVICEID_VALUE:
        case FSA9281_DEVICEID_VALUE:
            manual_sw1_reg = FSA9280_MANUALSW1_ADDRESS;
            vbus_state_reg = FSA9280_VBUS_VALID_STATUS_ADDRESS;
            break;
        case FSA9285_DEVICEID_VALUE:
            manual_sw1_reg = FSA9285_MANUAL_SWITCH_ADDRESS;
            vbus_state_reg = FSA9285_STATUS_ADDERSS;
            break;
        default:
            g_diag_usb_work_polling_flag = false;
            usb_diag_start_earphone_polling();
    ...
    ret = fsa928x_i2c_read(manual_sw1_reg, &read_manual_sw1_value);
    ...
    if(connected_audio_device == USB_DIAG_AUDIO_MODE1_CONNECTION)
    {
        write_manual_sw1_value &= FSA928X_MANUAL_SW1_VBUS_OUT_MASKB;
        write_manual_sw1_value |= (FSA928X_MANUAL_SW1_DM_AUDI_L_MASKB |FSA928X_MANUAL_SW1_DP_AUDI_R_MASKB);
        
        ret = fsa928x_i2c_read(vbus_state_reg, &vbus_state_value);
        ...
        if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == 0x00 )
        {
            
            usb_diag_set_path_vbus_in( USB_DIAG_VBUSIN_CON_MIC ,&write_manual_sw1_value);
            printk(KERN_INFO "[%s]:connected MODE1 with MIC!\n", __func__ );
        }
        else  if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == FSA928X_MANUAL_SW1_VBUS_BIT_MASKB )
        {
            usb_diag_set_path_vbus_in(USB_DIAG_VBUSIN_CON_VCHG ,&write_manual_sw1_value);
            printk(KERN_INFO "[%s]:connected MODE1 with VBUS!\n", __func__ );
        }

    }
    else if(connected_audio_device == USB_DIAG_AUDIO_MODE2_CONNECTION)
    {
        write_manual_sw1_value &= FSA928X_MANUAL_SW1_VBUS_OUT_MASKB;
        write_manual_sw1_value |= (FSA928X_MANUAL_SW1_DM_AUDI_L_MASKB |FSA928X_MANUAL_SW1_DP_OPEN_MASKB);
        ...
        if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == 0x00 )
        {
            ...
            usb_diag_set_path_vbus_in( USB_DIAG_VBUSIN_CON_MIC ,&write_manual_sw1_value);
            printk(KERN_INFO "[%s]:connected MODE2 with MIC!\n", __func__ );
        }
        else if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == FSA928X_MANUAL_SW1_VBUS_BIT_MASKB ) 
        {
             usb_diag_set_path_vbus_in(USB_DIAG_VBUSIN_CON_VCHG,&write_manual_sw1_value );
            printk(KERN_INFO "[%s]:connected MODE2 with VBUS!\n", __func__ );
        }        
    }
    else if(connected_audio_device == USB_DIAG_AUDIO_MODE3_CONNECTION)
    {
        write_manual_sw1_value &= FSA928X_MANUAL_SW1_VBUS_OUT_MASKB;
        write_manual_sw1_value |= (FSA928X_MANUAL_SW1_DM_AUDI_L_MASKB |FSA928X_MANUAL_SW1_DP_OPEN_MASKB);
        ...
        if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == 0x00 )
        {
            ...
            usb_diag_set_path_vbus_in( USB_DIAG_VBUSIN_OPEN, &write_manual_sw1_value );
            printk(KERN_INFO "[%s]:connected MODE3 with VBUSOPEN!\n", __func__ );
        }
        else  if( (vbus_state_value & FSA928X_MANUAL_SW1_VBUS_BIT_MASKB) == FSA928X_MANUAL_SW1_VBUS_BIT_MASKB ) 
        {
            usb_diag_set_path_vbus_in(USB_DIAG_VBUSIN_CON_VCHG, &write_manual_sw1_value );
            printk(KERN_INFO "[%s]:connected MODE3 with VBUS!\n", __func__ );
        }        
    }
    else if(connected_audio_device == USB_DIAG_NOT_AUDIO_CONNECTION)
    {
        write_manual_sw1_value &= FSA928X_MANUAL_SW1_VBUS_OUT_MASKB;
        write_manual_sw1_value |= (FSA928X_MANUAL_SW1_DM_HOST_MASKB |FSA928X_MANUAL_SW1_DP_HOST_MASKB);
        usb_diag_set_path_vbus_in(USB_DIAG_VBUSIN_CON_VCHG, &write_manual_sw1_value );
        printk(KERN_INFO "[%s]:connected NOT AUDIO(PC)!\n", __func__ );
    }else
    {
        printk(KERN_INFO "[%s]:No Action!\n", __func__ );
    }
    if(read_manual_sw1_value != write_manual_sw1_value)
    {
        ret = fsa928x_i2c_write(manual_sw1_reg, write_manual_sw1_value);
        printk(KERN_INFO "[%s]:manual_sw1 changed write = %d, read = %d \n", __func__, write_manual_sw1_value , read_manual_sw1_value );
        if ( ret == FSA928X_I2C_TRANSFER_NG )             
        {
            g_diag_usb_work_polling_flag = false;
            usb_diag_start_earphone_polling();
...
{
    int ret = FSA928X_I2C_TRANSFER_NG;      
    unsigned char adc_value = 0xff;
    unsigned char devicetype1_value = 0xff;
    unsigned char devicetype2_value = 0xff;
    unsigned char venderID = 0xff;
    usb_diag_connected_audio_device_enum ret_connected_audio_device = USB_DIAG_NOT_AUDIO_CONNECTION; 
    
    ret = fsa928x_i2c_read(FSA928X_DEVICE_ID_ADDRESS, &venderID);
    
    if ( ret == FSA928X_I2C_TRANSFER_NG)             
    {
        return ( USB_DIAG_ERROR_CONNECTION );
    }
    ...
    switch (venderID)
    {
        ...
        case FSA9281_DEVICEID_VALUE:
            ...
            fsa928x_i2c_read(FSA9280_ADC_ADDRESS, &adc_value);
            fsa928x_i2c_read(FSA9280_DEVICE_TYPE1_ADDRESS, &devicetype1_value);
            fsa928x_i2c_read(FSA9280_DEVICE_TYPE2_ADDRESS, &devicetype2_value);
            printk(KERN_INFO "[%s]:adc = %d , devicetype1 = %d, devicetype2 = %d\n", __func__, adc_value, devicetype1_value, devicetype2_value);
            
            if((adc_value == FSA9280_ADC_VALUE_MCPC_MODE1)  &&
            (devicetype1_value == 0) &&
            ((devicetype2_value & FSA9280_DEVTYPE2_USBBOOT_ON_MASKB) != 0))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE1_CONNECTION;
            
            }
            
            else if((adc_value == FSA9280_ADC_VALUE_MCPC_MODE2) &&
            ((devicetype1_value & FSA9280_DEVTYPE1_AUDIOTYPE_MASKB) != 0) &&
            (devicetype2_value == 0))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE2_CONNECTION;
                
            }
            
            else if((adc_value == FSA9280_ADC_VALUE_MCPC_MODE3)  &&
            (devicetype1_value == 0) &&
            ((devicetype2_value & FSA9280_DEVTYPE2_UARTBOOT_OFF_MASKB) != 0))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE3_CONNECTION;
            }
            break;
        case FSA9285_DEVICEID_VALUE:
            fsa928x_i2c_read(FSA9285_ADC_ADDRESS, &adc_value);
            
            if((adc_value == FSA9285_ADC_VALUE_MCPC_MODE1))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE1_CONNECTION;
            }
            
            else if((adc_value == FSA9285_ADC_VALUE_MCPC_MODE2))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE2_CONNECTION;
            }
            
            else if((adc_value == FSA9285_ADC_VALUE_MCPC_MODE3))
            {
                ret_connected_audio_device = USB_DIAG_AUDIO_MODE3_CONNECTION;
            }
            break;
    ...
    return (ret_connected_audio_device);
}
...
int usb_diag_set_path_vbus_in(usb_diag_vbus_in_path_enum usb_diag_vbus_in_path, unsigned char* manual_sw1_value)
{
    ...
    switch (venderID)
    {
        case FSA9280A_DEVICEID_VALUE:
        case FSA9281_DEVICEID_VALUE:
            reg = FSA9280_MANUALSW1_ADDRESS;
            break;
        case FSA9285_DEVICEID_VALUE:
            reg = FSA9285_MANUAL_SWITCH_ADDRESS;
            break;
        default:
            return ( FSA928X_I2C_TRANSFER_NG );
    }
    
    switch(usb_diag_vbus_in_path)
    {
        
        case USB_DIAG_VBUSIN_OPEN:
            *manual_sw1_value &= (~FSA928X_MANUAL_SW1_VBUS_OUT_MASKB);   
            break;
            
        
        case USB_DIAG_VBUSIN_CON_MIC:
            *manual_sw1_value &= (~FSA928X_MANUAL_SW1_VBUS_OUT_MASKB);   
            *manual_sw1_value |= FSA928X_MANUAL_SW1_VBUS_MIC_SET;        
            break;
        
        case USB_DIAG_VBUSIN_CON_VCHG:
            *manual_sw1_value |= (FSA928X_MANUAL_SW1_VBUS_OUT_MASKB);   
            break;
        
        
        default:
            printk(KERN_ERR "[%s]unknown request! vbus_in_path == %d\n", __func__, usb_diag_vbus_in_path);
            return ( FSA928X_I2C_TRANSFER_NG );
    }
    return (ret);
}
...
int usb_set_detect_earphone_switch(unsigned char* detect_earphone_switch)
{
    ...
    if((*detect_earphone_switch == USB_DETECT_EARPHONE_SWITCH_START ||
       *detect_earphone_switch == USB_DETECT_EARPHONE_SWITCH_STOP) )
    {
        g_usb_earphone_switch_detect_flg = *detect_earphone_switch;
    }
    printk(KERN_ERR "[%s] g_usb_earphone_switch_detect_flg = %d, g_usb_audio_flg = %d\n",__func__, g_usb_earphone_switch_detect_flg, g_usb_audio_flg);
    switch(*detect_earphone_switch)
    {
        case USB_DETECT_EARPHONE_SWITCH_START:
            if(g_usb_audio_flg == true)
            {
                fsa928x_start_adc_polling();
                printk(KERN_ERR "[%s] fsa928x_start_adc_polling called",__func__);
            }
            ret = 0;
            break;
        case USB_DETECT_EARPHONE_SWITCH_STOP:
            if(g_usb_audio_flg == true)
            {
...
int usb_diag_dev_file_ioctl(struct inode *inode, struct file *filp,
                            unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd){
        case IOCTL_USB_DIAG_FS_ENUM:
            usb_diag_cmd_out.rsp_data[0] = usb_fs_mode_configured();
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ));
            break;

        case IOCTL_USB_DIAG_HS_ENUM:
            usb_diag_cmd_out.rsp_data[0] = usb_hs_mode_configured();
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        ...
        case IOCTL_USB_DIAG_GET_CONNECT_STATUS:        
            usb_diag_cmd_out.rsp_data[0] = usb_diag_read_connection();
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        case IOCTL_USB_DIAG_REGULATE_SWITCH:           
            ret = copy_from_user(ioctl_data, (unsigned char*)arg, sizeof( ioctl_data ));
            if( ret != 0 )
            {
                break;
            }
            usb_diag_cmd_out.rsp_data[0] = usb_diag_write_regulation(ioctl_data);
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        case IOCTL_USB_DIAG_GET_REGISTER_VALUE:       
            ret = copy_from_user(ioctl_data, (unsigned char*)arg, sizeof( ioctl_data ));
            if( ret != 0 )
            {
                break;
            }
            usb_diag_cmd_out.rsp_data[0] = usb_diag_read_register(ioctl_data);
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        case IOCTL_USB_DIAG_SET_REGISTER_VALUE:       
            ret = copy_from_user(ioctl_data, (unsigned char*)arg, sizeof( ioctl_data ));
            if( ret != 0 )
            {
                break;
            }
            usb_diag_cmd_out.rsp_data[0] = usb_diag_write_register(ioctl_data);
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        ...
        case IOCTL_USB_GET_HEADSET_INFO:
            ret = copy_to_user( (usb_headset_info_type*)arg, &g_usb_headset_info,
                                 sizeof( usb_headset_info_type ) );
            break;


        case IOCTL_USB_SET_DETECT_EARPHONE_SWITCH:
            ret = copy_from_user(ioctl_data, (unsigned char*)arg, sizeof( ioctl_data ));
            if( ret != 0 )
            {
                break;
            }
            usb_diag_cmd_out.rsp_data[0] = usb_set_detect_earphone_switch(ioctl_data);
            ret = copy_to_user( usb_diag_cmd_in, &usb_diag_cmd_out,
                                 sizeof( struct usb_diag_ioctl_cmd ) );
            break;
        ...
        case IOCTL_USB_SET_MIC_DEC_WAITTIME:
            ret = copy_from_user(&g_usb_mic_dec_waittime, (unsigned char*)arg, sizeof( g_usb_mic_dec_waittime ));
            if( ret != 0 )
            {
                printk(KERN_ERR "[%s] IOCTL_USB_SET_MIC_DEC_WAITTIME:copy_from_user failed\n", __func__);
            }
            printk(KERN_ERR "[%s] IOCTL_USB_SET_MIC_DEC_WAITTIME: g_usb_mic_dec_waittime = %x\n", __func__, g_usb_mic_dec_waittime);
...
void usb_sw_otg_set_power(unsigned set_power){
    struct usb_info *u_inf = the_usb_info;
    if(u_inf != NULL)
    {
        otg_set_power(u_inf->xceiv, set_power);
    }
    else
    {
        printk(KERN_INFO "[%s] u_inf = NULL \n", __func__);
    }
}
...
static int __init init(void)
{
    ...
    usb_connect_boot_flag = 1;

	ret = comm_usb_init(android_callback, NULL);
	if (ret) {
...
static void __exit cleanup(void)
{
    ...
    dev_t devno = MKDEV( dev_num_major, 0 );
    ...
    device_destroy( usb_diag_class, usb_diag_devno );
    class_destroy( usb_diag_class );

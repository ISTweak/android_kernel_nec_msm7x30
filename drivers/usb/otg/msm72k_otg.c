...
typedef enum {
  PMIC_USB_AUDIO_TYPE_AUDIO,        
  PMIC_USB_AUDIO_TYPE_OTHER,        
  PMIC_USB_AUDIO_TYPE_DISCONNECTED, 
} PMIC_USB_AUDIO_TYPE;



bool g_pmic_usb_audio_gpio_stat = PMIC_GPIO_LOW_VALUE;
boolean usb_vbus_flg = false;
...
void FSA_set_otg_workable(int online);
int otg_workable = -1;
...
void FSA_set_otg_workable(int online)
{
	...
	wake_lock(&dev->wlock);
	queue_work(dev->wq, &dev->sm_work);
	pr_debug("[%s] B_SESS_VLD: %d , ID: %d\n", __func__, test_bit(B_SESS_VLD, &dev->inputs), test_bit(ID, &dev->inputs));
...
static void fsa928x_usbsw(struct work_struct *w)
{
    fsa928x_main();                               
}
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&usbsw_work, fsa928x_usbsw);         

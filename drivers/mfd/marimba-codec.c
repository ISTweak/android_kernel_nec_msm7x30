...
#include "../../arch/arm/mach-msm/qdsp5v2/NvRegister.h"

#define DIGITAL_RX_LEFT_GAIN_MAX   (signed int)0x00000014     
#define DIGITAL_RX_LEFT_GAIN_MIN   (signed int)0xFFFFFFAC     
#define DIGITAL_TX_LEFT_GAIN_MAX   (signed int)0x0000001E     
#define DIGITAL_TX_LEFT_GAIN_MIN   (signed int)0xFFFFFFBE     
...
int adie_codec_write(u8 reg, u8 mask, u8 val)
...
EXPORT_SYMBOL(adie_codec_write);	
...
static unsigned char* rx_device_name = "";
static unsigned char* tx_device_name = "";
...
static signed int profile_rx_gain = 0x00;
static signed int profile_tx_gain = 0x00;
...
static u8 rx_gain_calc_adjust_value(signed int val, signed int adjust)
{
    signed int tmp;
    ...
    if (tmp > DIGITAL_RX_LEFT_GAIN_MAX) {
        tmp = DIGITAL_RX_LEFT_GAIN_MAX;
    ...
    else if (tmp < DIGITAL_RX_LEFT_GAIN_MIN) {
        tmp = DIGITAL_RX_LEFT_GAIN_MIN;
...
static u8 tx_gain_calc_adjust_value(signed int val, signed int adjust)
{
    signed int tmp;
    ...
    if (tmp > DIGITAL_TX_LEFT_GAIN_MAX) {
        tmp = DIGITAL_TX_LEFT_GAIN_MAX;
    ...
    else if (tmp < DIGITAL_TX_LEFT_GAIN_MIN) {
        tmp = DIGITAL_TX_LEFT_GAIN_MIN;
...
{
    signed int rd, adjust;
    unsigned char wd;

    adjust = data;

    
    rd = adie_codec_get_profile_rx_gain();

    wd = rx_gain_calc_adjust_value(rd, adjust);

    
    adie_codec_write(0x8a, 0x04, 0x00);

    
    adie_codec_write(0x84, 0xFF, wd);

    
    adie_codec_write(0x8a, 0x04, 0x04);
}
...
void adie_tx_gain_adjust(signed char data)
{
    ...
    rd = adie_codec_get_profile_tx_gain();

    wd = tx_gain_calc_adjust_value(rd, adjust);

    
    adie_codec_write(0x8a, 0x40, 0x00);

    
    adie_codec_write(0x86, 0xFF, wd);

    
    adie_codec_write(0x8a, 0x40, 0x40);
}
						...
						u32 state)
{
	...
	while (!loop_exit) {
		...
		case ADIE_CODEC_ACTION_ENTRY:
            ...
            if(reg == 0x84) {
                if( strcmp(adie_codec_get_rx_device_name(), "handset_rx") == 0) {
                    
                    profile_rx_gain = (signed char)val; 
                    adjust          = (signed int)NvRegister_GetAdieGain_AdieCodecRxGain();
                    work_val = (signed char)val;





                    val      = rx_gain_calc_adjust_value( work_val, adjust );
                ...
                if(strcmp(adie_codec_get_tx_device_name(), "handset_tx") == 0) {
                     
                    
                    profile_tx_gain = (signed char)val; 
                    adjust          = (signed int)NvRegister_GetAdieGain_AdieCodecTxGain();
                    work_val = (signed char)val;




                    val      = tx_gain_calc_adjust_value( work_val, adjust );

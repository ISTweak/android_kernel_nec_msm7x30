...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	if (ret) {
        ...
        i2c_notify_alarm( msgs->addr, I2C_ALARM_BUS_BUSY, 0 );
	...
	while (rem) {
		...
		do {
			...
			if (!timeout) {
				...
				i2c_notify_alarm( msgs->addr, I2C_ALARM_TRANS_TIMEOUT, 0 );
			...
			if (dev->err) {
				if (dev->err > 0 &&
					...
					dev->msg->addr);
					i2c_notify_alarm( msgs->addr, I2C_ALARM_SLAVE_NOT_CONNECT, 0 );
				} else if (dev->err < 0) {
					dev_err(dev->dev,
					"QUP data xfer error %d\n", dev->err);
					i2c_notify_alarm( msgs->addr, I2C_ALARM_TRANS_FAILURE, dev->err );
					ret = dev->err;
					goto out_err;
				}
				ret = -dev->err;

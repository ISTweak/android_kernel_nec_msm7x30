...
struct msm_snddev_info * msm_snddev_get_dev_info(const char * name)
{
	int i;
	struct msm_snddev_info * ret_dev_info = NULL;

	if ( name == NULL ) return ret_dev_info;

	mutex_lock(&session_lock);

	for ( i = 0 ; i < audio_dev_ctrl.num_dev ; i++ ) {
		if ( strcmp( name , audio_dev_ctrl.devs[i]->name ) == 0 ) {
			ret_dev_info = audio_dev_ctrl.devs[i];
			break;
		}
	}
...
EXPORT_SYMBOL(msm_snddev_get_dev_info);
...
extern unsigned char g_snd_amp_sp_state;

static ssize_t show_snddev_nec_ext_amp( struct device *d, struct device_attribute *attr, char *buf )
{
  return sprintf( buf, "%02d", g_snd_amp_sp_state );
}

DEVICE_ATTR( snddev_nec_ext_amp, 0644, show_snddev_nec_ext_amp, NULL );
...
static int __init audio_dev_ctrl_init(void)
{
  ...
  ret = misc_register(&audio_dev_ctrl_misc);
  ret = device_create_file( audio_dev_ctrl_misc.this_device, &dev_attr_snddev_nec_ext_amp );

  if (ret)
  {
    printk( KERN_ERR "[yda160] sound_amp_INIT : FAIL to device_create_file \n");
  }
  return ret;
...
static void __exit audio_dev_ctrl_exit(void)
{
  ...
  device_remove_file( audio_dev_ctrl_misc.this_device, &dev_attr_snddev_nec_ext_amp );

...
static int voice_cmd_nv_get(void);
			...
			void *private_data)
{
	...
	int dev = NV_REGISTER_VE004_STATE_OFF;
	...
	case AUDDEV_EVT_DEV_CHG_VOICE:
        ...
        NvRegister_SetVoiceState( NV_REGISTER_VOICE_STATE_ON );





		if( NV_REGISTER_VE004_STATE_ON == NvRegister_GetAkPowerState() ){
			
			
            ak7837a_power_off();

			dev = ak7837a_getCurrentDevice();
			if(dev == AK7837A_PROC_DEVICE_SPEAKER) {
				
				ak7837a_power_on_speaker_stereo();



			}else{
				
				ak7837a_power_on_handset();
	...
	case AUDDEV_EVT_END_VOICE:
		...
		if ((v->dev_state == DEV_READY) ||
			...
			if (atomic_read(&v->rel_start_flag)) {
				...
				NvRegister_SetVoiceState( NV_REGISTER_VOICE_STATE_OFF );	
...
extern void NvRegister_PrintData(void);
				...
				void *evt_buf, u32 len)
{
    ...
    case EVENT_NV_GET:
        ...
        NvRegister_SetNvRegister( ((struct voice_nv *)evt_buf)->stNvRegister );
...
static int voice_cmd_nv_get(void)
{
    ...
    err = dalrpc_fcn_5(VOICE_DALRPC_CMD, v->handle, &hdr, sizeof(struct voice_header));

    if (err) {
        MM_ERR("<DRVSOUND> voice_cmd_nv_get : Nv Get command failed (Error Code = %d)\n", err);
    }

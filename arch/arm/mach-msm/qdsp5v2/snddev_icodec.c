...
extern struct adie_codec_dev_profile ihs_stereo_rx_profile;
...
extern struct adie_codec_dev_profile iearpiece_profile;
...
extern struct adie_codec_dev_profile imic_profile;
...
extern struct adie_codec_dev_profile ispeaker_rx_profile;
...
extern struct adie_codec_dev_profile ispeaker_tx_profile;
...
extern struct adie_codec_dev_profile ihs_mono_tx_profile;
...
extern struct adie_codec_dev_profile iusb_stereo_rx;
...
static int snddev_icodec_open_rx(struct snddev_icodec_state *icodec)
{
    ...
    adie_codec_set_rx_device_name((unsigned char*)icodec->data->name);
    ...
    MM_INFO("snddev_icodec_open_rx : <REG_CTRLT> start name = %s\n", icodec->data->name);
    ...
    MM_INFO("snddev_icodec_open_rx : <REG_CTRLT> end   name = %s\n", icodec->data->name);
...
error_afe:
    ...
    adie_codec_set_rx_device_name((unsigned char*)"");
...
static int snddev_icodec_open_tx(struct snddev_icodec_state *icodec)
{
    ...
    MM_INFO("snddev_icodec_open_tx : <REG_CTRLT> start name = %s\n", icodec->data->name);
    ...
    adie_codec_set_tx_device_name((unsigned char*)icodec->data->name);
    ...
    MM_INFO("snddev_icodec_open_tx : <REG_CTRLT> end   name = %s\n", icodec->data->name);
...
error_afe:
    ...
    adie_codec_set_tx_device_name((unsigned char*)"");
...
static int snddev_icodec_close_rx(struct snddev_icodec_state *icodec)
{
    ...
    MM_INFO("snddev_icodec_close_rx : <REG_CTRLT> start name = %s\n", icodec->data->name);
    ...
    MM_INFO("snddev_icodec_close_rx : <REG_CTRLT> end   name = %s\n", icodec->data->name);
...
static int snddev_icodec_close_tx(struct snddev_icodec_state *icodec)
{
    ...
    MM_INFO("snddev_icodec_close_tx : <REG_CTRLT> start name = %s\n", icodec->data->name);
    ...
    MM_INFO("snddev_icodec_close_tx : <REG_CTRLT> end   name = %s\n", icodec->data->name);
...
static int snddev_icodec_open(struct msm_snddev_info *dev_info)
{
	...
	if ( Diag_snd_calib_data_load_func ) {
		
		Diag_snd_calib_data_load_func();
		
		Diag_snd_calib_data_load_func = NULL;
	}
...
static int snddev_icodec_open_rx_diag(struct snddev_icodec_state *icodec)
{
	int trc;
	struct msm_afe_config afe_config;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
...
static int snddev_icodec_open_tx_diag(struct snddev_icodec_state *icodec)
{
	int trc;
	struct msm_afe_config afe_config;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;;

printk("[diag] diag tx open : %s\n", icodec->data->name);
...
static int snddev_icodec_close_rx_diag(struct snddev_icodec_state *icodec)
{
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
...
static int snddev_icodec_close_tx_diag(struct snddev_icodec_state *icodec)
{
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
	afe_disable(AFE_HW_PATH_CODEC_TX);

printk("[diag] diag tx close : %s\n", icodec->data->name);
...
static int snddev_icodec_open_diag(struct msm_snddev_info *dev_info)
{
	int rc = 0;
	struct snddev_icodec_state *icodec;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
	...
	if (icodec->data->capability & SNDDEV_CAP_RX) {
		...
		rc = snddev_icodec_open_rx_diag(icodec);
		if (!IS_ERR_VALUE(rc))
			drv->rx_active = 1;
		mutex_unlock(&drv->rx_lock);
	} else {
		...
		rc = snddev_icodec_open_tx_diag(icodec);
		if (!IS_ERR_VALUE(rc))
			drv->tx_active = 1;
		mutex_unlock(&drv->tx_lock);
	}
...
static int snddev_icodec_close_diag(struct msm_snddev_info *dev_info)
{
	int rc = 0;
	struct snddev_icodec_state *icodec;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
	...
	if (icodec->data->capability & SNDDEV_CAP_RX) {
		...
		rc = snddev_icodec_close_rx_diag(icodec);
		if (!IS_ERR_VALUE(rc))
			drv->rx_active = 0;
		mutex_unlock(&drv->rx_lock);
	} else {
		...
		rc = snddev_icodec_close_tx_diag(icodec);
		if (!IS_ERR_VALUE(rc))
			drv->tx_active = 0;
		mutex_unlock(&drv->tx_lock);
	}
...
static struct msm_snddev_info* diag_lp_tx[30];


static struct msm_snddev_info* diag_lp_rx[30];

static struct msm_snddev_info* diag_now_tx;
static struct msm_snddev_info* diag_now_rx;
...
{	
	...
	printk("[diag] qtr action ctrl. START\n");
	switch( data->type)
	{
		case 0x00:
			
			
			printk("[diag] qtr action ctrl. close codec\n");
			if(diag_now_tx != NULL)	snddev_icodec_close_diag(diag_now_tx);
			if(diag_now_rx != NULL)	snddev_icodec_close_diag(diag_now_rx);
		...
		case 0x01:
			...
			printk("[diag] qtr action ctrl. tx open\n");

			printk("diag_lp_tx[%d]\n", (int)data->tx_dev);

			
			diag_now_tx = diag_lp_tx[ (int)data->tx_dev ];
			
			if(diag_now_tx == NULL) return 1;
			
			
			snddev_icodec_open_diag(diag_now_tx);
			
			
			break;
		case 0x10:
			
			printk("[diag] qtr action ctrl. rx open\n");

			printk("diag_lp_rx[%d]\n", (int)data->rx_dev);

			
			diag_now_rx = diag_lp_rx[ (int)data->rx_dev ];
			
			if(diag_now_rx == NULL) return 1;
			
			
			snddev_icodec_open_diag(diag_now_rx);
			
			break;
		case 0x11:
			
			printk("[diag] qtr action ctrl. tx rx open\n");
	...
	printk("[diag] qtr action ctrl.  END\n");
	return 0;
}
EXPORT_SYMBOL(Diag_Audio_qtr_action);
...
{
	struct adie_codec_dev_profile * rx_profile = NULL;
	struct adie_codec_dev_profile * tx_profile = NULL;
	struct snddev_icodec_drv_state *drv = &snddev_icodec_drv;
	const char * dev_name_tx = NULL, * dev_name_rx = NULL;
	int devid;
	
	
	extern int adie_codec_write(u8 reg, u8 mask, u8 val);
	
	printk("[diag] qtr loopback. START\n");
	...
	if((devid % 16) == 1)
	{
		printk("[diag] qtr loopback. RX=1\n");
		rx_profile = & iearpiece_profile;
		dev_name_rx = "handset_rx";
	}
	else if((devid % 16) == 2)
	{
		printk("[diag] qtr loopback.RX=2\n");

		rx_profile   = & ihs_stereo_rx_profile;
		dev_name_rx  = "headset_stereo_rx";
	...
	{
		printk("[diag] qtr loopback.RX=3\n");
		rx_profile = & iusb_stereo_rx;
		dev_name_rx = "usb_stereo_rx";
	}
	...
	if((devid >> 4) == 1)
	{
		printk("[diag] qtr loopback.TX=1\n");
		tx_profile = & imic_profile;
		dev_name_tx = "handset_tx";
	}
	else if((devid >> 4) == 2)
	{
		printk("[diag] qtr loopback.TX=2\n");
		tx_profile = & ispeaker_tx_profile;
		dev_name_tx = "speaker_mono_tx";
	}
	else if((devid >> 4) == 3)
	{
		printk("[diag] qtr loopback.TX=3\n");
		tx_profile = & ihs_mono_tx_profile;
		dev_name_tx = "headset_mono_tx";
	}
	...
	if((tx_profile == NULL || rx_profile == NULL) && devid != 0)
	{
		printk("[diag] qtr loopback failed.\n");
		return 1;
	}
	
	
	if (devid != 0) {
		printk("[diag] qtr loopback enable LP.\n");
		...
		clk_set_rate(drv->rx_mclk,
			SNDDEV_ICODEC_CLK_RATE(48000));
		clk_enable(drv->rx_mclk);
		clk_enable(drv->rx_sclk);

		printk("%s: configure ADIE RX path\n", __func__);
		
		adie_codec_open(rx_profile, &debugfs_rx_adie);



        
        adie_codec_set_rx_device_name((unsigned char*)dev_name_rx);



		adie_codec_setpath(debugfs_rx_adie, 48000, 256);
		adie_codec_proceed_stage(debugfs_rx_adie,
		ADIE_CODEC_DIGITAL_ANALOG_READY);

		printk("%s: Enable Handset Mic bias\n", __func__);
		...
		printk("%s: configure ADIE TX path\n", __func__);
		
		adie_codec_open(tx_profile, &debugfs_tx_adie);



        
        adie_codec_set_rx_device_name((unsigned char*)dev_name_tx);



		adie_codec_setpath(debugfs_tx_adie, 48000, 256);
		adie_codec_proceed_stage(debugfs_tx_adie,
		ADIE_CODEC_DIGITAL_ANALOG_READY);
		
		
		adie_codec_write(0x91, 0xFF, 0x01);
	
	} else {
		printk("[diag] qtr loopback disable LP.\n");

		
		adie_codec_write(0x91, 0xFF, 0x00);
		adie_codec_proceed_stage(debugfs_rx_adie,
	...
	printk("[diag] qtr loopback END.\n");

	return 0;
}
EXPORT_SYMBOL(Diag_Audio_qtr_loopback);
...
char Diag_Audio_hsedbias_ctl( Diag_Audio_hsedbias_ctl_type *data )
{
	...
	switch(data->bias)
	{
		case 0x00:
			...
			printk("[diag] hsed bis ctrl : 0x00 0&1 OFF\n");
			...
			pmic_hsed_enable(PM_HSED_CONTROLLER_1,
			 PM_HSED_ENABLE_OFF);
		...
		case 0x01:
			...
			printk("[diag] hsed bis ctrl : 0x01 0 ON\n");
			pmic_hsed_enable(PM_HSED_CONTROLLER_0, PM_HSED_ENABLE_PWM_TCXO);
			break;
		case 0x10:
			
			printk("[diag] hsed bis ctrl : 0x00 1 ON\n");
		...
		default:
			...
			printk("[diag] hsed bis ctrl : illegal argument\n");
			return 1;
	}

	return 0;
}
EXPORT_SYMBOL(Diag_Audio_hsedbias_ctl);
...
{
    printk("[diag] rx gain adjust\n");

    adie_rx_gain_adjust(data);

    return 0;
}
EXPORT_SYMBOL(Diag_Audio_rx_gain_adjust);



signed char Diag_Audio_tx_gain_adjust(signed char data)
{
    printk("[diag] tx gain adjust\n");

    adie_tx_gain_adjust(data);

    return 0;
}
EXPORT_SYMBOL(Diag_Audio_tx_gain_adjust);
...
MM_ERR("snddev_icodec_probe : name = %s\n", pdata->name);
	...
	if(strcmp(dev_info->name, "handset_rx") == 0)
	{
		diag_lp_rx[1] = dev_info;
		diag_lp_rx[2] = dev_info;
		diag_lp_rx[3] = dev_info;
		diag_lp_rx[4] = dev_info;
		
	}
	else if(strcmp(dev_info->name, "headset_stereo_rx") == 0)
	{
		diag_lp_rx[7] = dev_info;
		diag_lp_rx[8] = dev_info;
		diag_lp_rx[9] = dev_info;
		diag_lp_rx[10] = dev_info;
		diag_lp_rx[11] = dev_info;
		
	}
	else if(strcmp(dev_info->name, "speaker_stereo_rx") == 0)
	{
		diag_lp_rx[14] = dev_info;
		diag_lp_rx[15] = dev_info;
		diag_lp_rx[16] = dev_info;
		diag_lp_rx[17] = dev_info;
		
	}
	
	else if(strcmp(dev_info->name, "handset_tx") == 0)
	{
		diag_lp_tx[1] = dev_info;
		diag_lp_tx[2] = dev_info;
		diag_lp_tx[3] = dev_info;
		diag_lp_tx[4] = dev_info;
		diag_lp_tx[5] = dev_info;
		
	}
	else if(strcmp(dev_info->name, "headset_mono_tx") == 0)
	{
		diag_lp_tx[7] = dev_info;
		diag_lp_tx[8] = dev_info;
		diag_lp_tx[9] = dev_info;
		diag_lp_tx[10] = dev_info;
		diag_lp_tx[11] = dev_info;
		
	}
	else if(strcmp(dev_info->name, "speaker_mono_tx") == 0)
	{
		diag_lp_tx[13] = dev_info;
		diag_lp_tx[14] = dev_info;
		diag_lp_tx[15] = dev_info;
		diag_lp_tx[16] = dev_info;
		diag_lp_tx[17] = dev_info;

...
#define MESSAGE_BUF_ADDRESS MSM_OEMCH_DB_UINIT_BASE

#define MESSAGE_BUF_SIZE_MAX (1024)

#define SYSLOG_BUF_ADDRESS (MSM_OEMCH_DB_UINIT_BASE + SZ_1M)

#define SYSLOG_BUF_SIZE_MAX (1<<17)
extern int nec_syslog(char *buf, int len);
extern int nec_pullout_pc(char *buf, int len);
...
static int nec_flg_already_cp_syslog = 0;





#define SMSM_APPS_ALARM_EVENT SMSM_MODEM_CONTINUE
...
static void handle_modem_crash(void)
{
	...
	*/
	...
	if(!nec_flg_already_cp_syslog)
    {
	    nec_syslog((char*)SYSLOG_BUF_ADDRESS,SYSLOG_BUF_SIZE_MAX);
		nec_flg_already_cp_syslog = 1;
    }
...
void nec_handle_fatalA(char *message,int message_len)
{
	...
	if(
		((ptr[0]=='A')&&(ptr[1]=='P')&&(ptr[2]=='P'))||
		((ptr[0]=='M')&&(ptr[1]=='O')&&(ptr[2]=='D'))
	)	 goto RESET_MODEM;
...
EXPORT_SYMBOL(nec_handle_fatalA);



void notify_alarmlog(void)
{
    uint32_t sts;
    sts = smsm_get_state( SMSM_APPS_STATE );
    if ( sts & SMSM_APPS_ALARM_EVENT ){
	smsm_change_state( SMSM_APPS_STATE, SMSM_APPS_ALARM_EVENT, 0 );
    } else {
	smsm_change_state( SMSM_APPS_STATE, 0, SMSM_APPS_ALARM_EVENT );
    }
}
EXPORT_SYMBOL(notify_alarmlog);

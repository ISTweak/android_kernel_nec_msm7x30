...
uint32_t hw_revision_read (void) {
	struct socinfo_v4  *p_socinfo_v4;
	p_socinfo_v4 = smem_alloc(SMEM_HW_SW_BUILD_ID, sizeof(struct socinfo_v4));

	if (!p_socinfo_v4) {
		pr_info("Type of SOCINFO is not v4. Can't return HW REVION!\n");
		return HW_REVISION_READ_ERR;
	}
	return p_socinfo_v4->platform_version;
}

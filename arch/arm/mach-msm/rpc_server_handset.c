...
#define HS_STEREO_HEADSET3_K	0x3A
#define HS_STEREO_HEADSET4_K	0x3B
...
#define JACKOUT_KIND  0x00
#define HEADHONE_KIND 0x01
#define EARPHONE_KIND 0x02  
#define USBAUDIO_KIND 0x04
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_STEREO_HEADSET3_K, SW_STEREO_HEADSET3),
	KEY(HS_STEREO_HEADSET4_K, SW_STEREO_HEADSET4),
...
static int headphone_usbaudio = JACKOUT_KIND;    
static int old_headphone_usbaudio = JACKOUT_KIND;
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	int key, temp_key_code, key_mask;
...
void hs_report_headphone_usbaudio(hs_headphone_usbaudio_state key,bool state)
{
	...
	case HEADPHONE_3:
		if(state){
			headphone_usbaudio |= EARPHONE_KIND;
		}else{
			if(headphone_usbaudio & EARPHONE_KIND){
				headphone_usbaudio ^= EARPHONE_KIND;
			}
		}
		break;
	case HEADPHONE_4:
		if(state){
			headphone_usbaudio |= HEADHONE_KIND;
		}else{
			if(headphone_usbaudio & HEADHONE_KIND){
				headphone_usbaudio ^= HEADHONE_KIND;
			}
		}
		break;
	case USBAUDIO:
		if(state){
			headphone_usbaudio |= USBAUDIO_KIND;
		}else{
			if(headphone_usbaudio & USBAUDIO_KIND){
				headphone_usbaudio ^= USBAUDIO_KIND;
			}
		}
		break;
	...
	if(headphone_usbaudio != old_headphone_usbaudio){
		old_headphone_usbaudio = headphone_usbaudio;
		
		report_headset_switch(hs->ipdev, SW_HEADPHONE_INSERT,headphone_usbaudio);
	}
	return;
}
EXPORT_SYMBOL(hs_report_headphone_usbaudio);

unsigned char hs_key_cmd(int *val)
{
	input_report_key(hs->ipdev,val[1],val[2]);
	input_sync(hs->ipdev);
	return 1;
}
EXPORT_SYMBOL(hs_key_cmd);
...
static int __devinit hs_probe(struct platform_device *pdev)
{
    ...
    input_set_capability(ipdev, EV_SW, HS_STEREO_HEADSET3_K);
    input_set_capability(ipdev, EV_SW, HS_STEREO_HEADSET4_K);
    ...
    printk(KERN_ERR"[hs_probe]test_DEBUG-USB\n");

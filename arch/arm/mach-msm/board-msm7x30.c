...
#include "qdsp5v2/ak7837a_i2c_driver.h"
...
#include "../drivers/mmc/card/block.h"
...
#define GPIO_SD_DET  145  



#define PMIC_GPIO_SDC4_EN  17  

#define PMIC_GPIO_AUD_DDC_EN  23  
...
struct pm8058_gpio aud_ddc_drv = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
		.output_value   = 0,
		.pull           = PM_GPIO_PULL_NO,
	...
	rc = pm8058_gpio_config(PMIC_GPIO_AUD_DDC_EN, &aud_ddc_drv);
	if (rc) {
		pr_err("%s PMIC_GPIO_AUD_DDC_EN config failed\n",__func__);
		return rc;
	}
 ...
 static const unsigned int db7630_keymap[] = {
 	KEY(0, 0, KEY_SEARCH),
 	KEY(1, 0, KEY_SEND),
 	KEY(2, 0, KEY_1),
 	KEY(3, 0, KEY_4),
...
static struct matrix_keymap_data db7630_keymap_data = {
	.keymap_size	= ARRAY_SIZE(db7630_keymap),
	.keymap		= db7630_keymap,
};

static struct pmic8058_keypad_data db7630_keypad_data = {
	.input_name		= "db7630-keypad",
	...
	.row_hold_ns		= 91500,  
	.wakeup			= 1,
	.keymap_data		= &db7630_keymap_data,
};
...
static struct ak7837a_platform_data ak7837a_pdata = {
	.ak7837a_setup = NULL,
	.ak7837a_shutdown = NULL,
};
...
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
    ...
    {
		I2C_BOARD_INFO(AK7837A_I2C_DEVICE_NAME, AK7837A_I2C_SLAVE_ADDRESS),
		.platform_data = &ak7837a_pdata,
    }
};
...
static struct msm_camera_sensor_flash_data flash_ius006f = {
	.flash_type = MSM_CAMERA_FLASH_LED,
	.flash_src  = &msm_flash_src
};
static struct msm_camera_sensor_info msm_camera_sensor_ius006f_data = {
	.sensor_name    = "ius006f",
	.sensor_reset   = 23,
	.sensor_pwd     = 21,
	.vcm_pwd        = 0,
...
static struct platform_device msm_camera_sensor_ius006f = {
	.name      = "msm_camera_ius006f",
	.dev       = {
		.platform_data = &msm_camera_sensor_ius006f_data,
	},
};



static struct i2c_board_info msm_bd6082gcl_boardinfo[] __initdata = {
	{
		I2C_BOARD_INFO("msm_led_bd6082gcl", 0x76 ),
	},
};
...
static char *usb_functions_process[] = {
	"diag",
	"adb",
};
...
static struct android_usb_product usb_products[] = {
	...
	{
		.product_id	= 0x02EB,
		.num_functions	= ARRAY_SIZE(usb_functions_process),
		.functions	= usb_functions_process,
	},
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.version = 0x0100,
	.product_name  = "VE357",
	.manufacturer_name = "NEC Corporation",
...
static struct gp2ap012a00f_platform_data gp2ap012a00f_data = {
	.data = 0,
};
...
static u32 fsa928x_hw_setup(void)
{
	int rc = 0;

	struct pm8058_gpio fsa928x_gpio = {
			PM_GPIO_DIR_IN,				
			PM_GPIO_OUT_BUF_OPEN_DRAIN,	
			0,	 							
			PM_GPIO_PULL_NO,				
			PM_GPIO_VIN_L5, 				
			PM_GPIO_STRENGTH_NO,			
	...
	rc = pm8058_gpio_config(FSA928X_I2C_PM_GPIO_INTB, &fsa928x_gpio);
	printk(KERN_INFO "[FSA9280A] Called hw_setup: rc=%d\n", rc);

	return rc;
}

static struct fsa928x_platform_data fsa928x_pdata = {
	.fsa928x_setup = fsa928x_hw_setup,
};
...
static struct i2c_board_info msm_i2c_board_info[] = {
	...
	{
		I2C_BOARD_INFO("gp2ap012a00f", 0x44),
		.platform_data	 = &gp2ap012a00f_data,
	}
	...
	,{
		I2C_BOARD_INFO(FSA928X_I2C_DEVICE_NAME , FSA928X_I2C_SLAVE_ADDRESS),
		.platform_data = &fsa928x_pdata,
		.irq = PM8058_GPIO_IRQ(PMIC8058_IRQ_BASE, FSA928X_I2C_PM_GPIO_INTB),
	}
...
static struct i2c_board_info nim_boardinfo[] __initdata = {
    {
        I2C_BOARD_INFO("nim_ofdm", 0x6C),
    },
    {
        I2C_BOARD_INFO("nim_rf",   0x60),
    }
};
...
static struct msm_gpio synaptics_ncm_spi_gpio_config_data[] = {
	{ GPIO_CFG( 92, 0, GPIO_CFG_INPUT,   GPIO_CFG_PULL_UP, GPIO_CFG_2MA), "tpanel2_int" },
	{ GPIO_CFG(106, 0, GPIO_CFG_OUTPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "tp_pwr_en"   },
...
static int synaptics_ncm_hw_setup(struct device *dev)
{
	...
	rc = msm_gpios_request_enable(synaptics_ncm_spi_gpio_config_data,
								  ARRAY_SIZE(synaptics_ncm_spi_gpio_config_data));
...
{
	msm_gpios_disable_free(synaptics_ncm_spi_gpio_config_data,
		ARRAY_SIZE(synaptics_ncm_spi_gpio_config_data));
}
...
static struct msm_gpio qsd_spi_gpio_config_data[] = {
	...
	{ GPIO_CFG(89, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "spi_cs3" },
...
static int msm_fb_detect_panel(const char *name)
{
    ...
    if (!strncmp(name, "mddi_tmd_fwvga_pt", sizeof("mddi_tmd_fwvga_pt")-1))
        return 0;
    else
        return -ENODEV;
...
static int mddi_hitachi_ledctl_bl(int level)
{
	...
	if( level >= 1 ){
	    ret = bd6082gul_main_lcd_bright((unsigned char)(level-1));
	    if ((ret != BD6082GUL_LED_SET_OK) && (ret != BD6082GUL_LED_SET_RESERVED)) {
	        pr_err("%s: Level of LCD_bl brightness is error!(ret=0x%08x)\n",
	                __func__, ret);
	        return -EPERM;
	    }
	}
    ...
    if ((ret != BD6082GUL_LED_SET_OK) && (ret != BD6082GUL_LED_SET_RESERVED)) {
        pr_err("%s: Level of LCD_bl set is error!(ret=0x%08x)\n",
                __func__, ret);
        return -EPERM;
    }
...
static struct msm_panel_common_pdata mddi_hitachi_pdata = {
    ...
    .pmic_backlight = mddi_hitachi_ledctl_bl

};


static struct platform_device mddi_ta8851_device = {
	.name = "mddi_ta8851",
	.id   = 0,
	.dev    = {
		.platform_data = &mddi_hitachi_pdata,
	}
};
...
static struct msm_panel_common_pdata mddi_tmd_pdata = {
    .pmic_backlight = NULL
};

static struct platform_device mddi_nt35560_device = {
	.name = "mddi_nt35560",
	.id   = 0,
	.dev    = {
		.platform_data = &mddi_tmd_pdata,
	}
};
...
static struct platform_device led_drv_ic_leds = {
	.name   = "led-drv-ic",
	.id = -1,
};
...
#define MSM_MMC_DISABLE			0
#define MSM_MMC_ENABLE			1
#define MSM_GPIO_SDEX_RESET		125
#define MSM_VREG_GP10_LEVEL		2850
#define MMC_MN66829RF_BASE		0x8B000000
#define EBI2_CHIP_SELECT_CFG0	0xA0000000
#define EBI2_CS3_CFG_MASK		(1 << 5)
#define MSM_GP_NS_REG			0xAB80005C
#define MSM_GP_NS_REG_ENABLE	0x00000A06
#define MSM_GP_NS_REG_DISABLE	0x00000006

static struct msm_gpio mmc_mn66829rf_gpios[] = {
	{GPIO_CFG( 88, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "sdex_cs"},
	{GPIO_CFG( 98, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "sdex_clk"},
	{GPIO_CFG(125, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "sdex_reset"},
	{GPIO_CFG(126, 0, GPIO_CFG_INPUT,  GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), "sdex_irq"},
	{GPIO_CFG(145, 0, GPIO_CFG_INPUT,  GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "sdex_cd"},
	{GPIO_CFG(171, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_7"},
	{GPIO_CFG(172, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_6"},
	{GPIO_CFG(173, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_5"},
	{GPIO_CFG(174, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_4"},
	{GPIO_CFG(175, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_3"},
	{GPIO_CFG(176, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_2"},
	{GPIO_CFG(177, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), "ebi2_adr_1"},
};

static uint32_t mmc_mn66829rf_setup_vreg(int enable)
{
	uint32_t rc = 0;
	...
	if (enable) {
		rc = vreg_set_level(vreg_gp10, MSM_VREG_GP10_LEVEL);
		if (rc) {
			pr_err("%s: vreg_set_level(%s) = %d \n", __func__, "gp10", rc);
			return rc;
		}

		rc = vreg_enable(vreg_gp10);
		if (rc) {
			pr_err("%s: vreg_enable(%s) = %d \n", __func__, "gp10", rc);
		}
	} else {
		rc = vreg_disable(vreg_gp10);
		if (rc) {
			pr_err("%s: vreg_disable(%s) = %d \n", __func__, "gp10", rc);
		}
	}
...
static uint32_t mmc_mn66829rf_setup_clock(int enable)
{
	uint32_t rc = 0;
	uint32_t msm_gp_ns = 0;
	void __iomem *msm_gp_ns_addr = 0;

	msm_gp_ns_addr = ioremap(MSM_GP_NS_REG, sizeof(uint32_t));
	if (!msm_gp_ns_addr) {
		pr_err("%s: Could not remap %x\n", __func__, MSM_GP_NS_REG);
		return !msm_gp_ns_addr;
	}

	msm_gp_ns = (readl(msm_gp_ns_addr) & 0xFFFF0000);

	if (enable) {
		msm_gp_ns |= MSM_GP_NS_REG_ENABLE;
	} else {
		msm_gp_ns |= MSM_GP_NS_REG_DISABLE;
	}

	writel(msm_gp_ns, msm_gp_ns_addr);
	iounmap(msm_gp_ns_addr);
	return rc;
}

static uint32_t mmc_mn66829rf_host_init( void )
{
	uint32_t rc = 0;
	void __iomem *ebi2_addr = 0;

	
	rc = msm_gpios_request_enable(mmc_mn66829rf_gpios, ARRAY_SIZE(mmc_mn66829rf_gpios));
	if (rc) {
	...
	gpio_set_value(MSM_GPIO_SDEX_RESET, 0);

	
	ebi2_addr = ioremap(EBI2_CHIP_SELECT_CFG0, sizeof(uint32_t));
	if (!ebi2_addr) {
		pr_err("%s: Could not remap %x\n", __func__, EBI2_CHIP_SELECT_CFG0);
		rc = !ebi2_addr;
		goto err_setup;
	}
	writel(readl(ebi2_addr) | EBI2_CS3_CFG_MASK, ebi2_addr);
	iounmap(ebi2_addr);

	
	rc = mmc_mn66829rf_setup_clock(MSM_MMC_ENABLE);
	if (rc) {
		pr_err("%s: mmc_mn66829rf_setup_clock = %d \n", __func__, rc);
		goto err_setup;
	}

	
	gpio_set_value(MSM_GPIO_SDEX_RESET, 1);
	return rc;

err_setup:
	msm_gpios_disable_free(mmc_mn66829rf_gpios, ARRAY_SIZE(mmc_mn66829rf_gpios));
out:
	return rc;
}

static uint32_t mmc_mn66829rf_setup_power(struct device *dv, unsigned int mode)
{
	uint32_t rc = 0;

	switch (mode) {
	case MMC_HOST_INIT:
		rc = mmc_mn66829rf_host_init();
		break;

	case MMC_HOST_EXIT:
		mmc_mn66829rf_setup_clock(MSM_MMC_DISABLE);
		gpio_set_value(MSM_GPIO_SDEX_RESET, 0);
		msm_gpios_disable_free(mmc_mn66829rf_gpios, ARRAY_SIZE(mmc_mn66829rf_gpios));
		break;
	...
	case MMC_CARD_ON:
		rc = mmc_mn66829rf_setup_vreg(MSM_MMC_ENABLE);
		break;

	case MMC_CARD_OFF:
		mmc_mn66829rf_setup_vreg(MSM_MMC_DISABLE);
		break;
...
static unsigned int mmc_mn66829rf_slot_status(struct device *dev)
{
	return (unsigned int)gpio_get_value(GPIO_SD_DET);
}

static struct resource resources_mmc_mn66829rf[] = {
	{
		.start	= MMC_MN66829RF_BASE,
		.end	= MMC_MN66829RF_BASE + SZ_512 - 1,
		.flags	= IORESOURCE_MEM,
	},
	{
...
static struct mmc_platform_data mmc_mn66829rf_data = {
	.ocr_mask      = MMC_VDD_28_29,
	.translate_vdd = mmc_mn66829rf_setup_power,
	.mmc_bus_width = MMC_CAP_4_BIT_DATA,
	.status        = mmc_mn66829rf_slot_status,
	.status_irq    = MSM_GPIO_TO_INT(GPIO_SD_DET),
	.irq_flags     = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
};

struct platform_device mmc_mn66829rf_device = {
	.name          = "mmc_mn66829rf",
	.id            = 0,
	.num_resources = ARRAY_SIZE(resources_mmc_mn66829rf),
	.resource      = resources_mmc_mn66829rf,
	.dev = {
		.platform_data = &mmc_mn66829rf_data,
	},
};
...
&msm_camera_sensor_ius006f,




	&mmc_mn66829rf_device,
...
static struct msm_gpio sdc4_cfg_data_off[] = {
	{GPIO_CFG(58, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_clk"},
	{GPIO_CFG(59, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_cmd"},
	{GPIO_CFG(60, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_dat_3"},
	{GPIO_CFG(61, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_dat_2"},
	{GPIO_CFG(62, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_dat_1"},
	{GPIO_CFG(63, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), "sdc4_dat_0"},
};
...
static struct sdcc_gpio sdcc_cfg_data[] = {
	...
	{
		.cfg_data = sdc4_cfg_data_off,
		.size = ARRAY_SIZE(sdc4_cfg_data_off),
	},
...
static struct sdcc_vreg sdcc_vreg_data[5];
...
static uint32_t msm_sdcc_setup_gpio(int dev_id, unsigned int enable)
{
	...
	if (enable) {
		...
		if( dev_id == 4 )
		{
			if( test_bit( dev_id + 1, &gpio_sts ) != 0 )
			{
				pr_debug("%s: GPIO LOW OUT OFF\n", __func__ );

				clear_bit(dev_id + 1, &gpio_sts);
				msm_gpios_disable_free( sdcc_cfg_data[dev_id].cfg_data, sdcc_cfg_data[dev_id].size );
			}
		}
		...
		if( dev_id == 4 )
		{
			if( test_bit( dev_id + 1, &gpio_sts ) == 0 )
			{
				pr_debug("%s: GPIO LOW OUT\n",
					__func__ );

				set_bit(dev_id + 1, &gpio_sts);
				rc = msm_gpios_request_enable( sdcc_cfg_data[dev_id].cfg_data, sdcc_cfg_data[dev_id].size );

				if (rc)
				{
					printk(KERN_ERR "%s: Failed to turn on GPIOs for SD slot off (%d)\n",
						__func__,  dev_id + 1 );
				}
				else
				{
					for( count = 0; count < ARRAY_SIZE(sdc4_cfg_data_off); count++ )
					{
						ret = gpio_direction_output( GPIO_PIN(sdc4_cfg_data_off[count].gpio_cfg), 0 );
						if( ret != 0 )
						{
							printk(KERN_ERR "%s: Failed to turn on GPIO (%d) output\n", __func__, count + 58 );
						}
					}
				}
			}
...
static uint32_t msm_sdcc_setup_vreg(int dev_id, unsigned int enable)
{
	...
	static int enabled_once[] = {0, 0, 0, 0, 0};
...
static uint32_t msm_sdcc_setup_power(struct device *dv, unsigned int vdd)
{
	...
	if( (vdd ? 1 : 0) == 0 )
	{
		...
		if (pdev->id == 4) 
		{
			...
			if( rc == 0 )
			{
				rc = msm_sdcc_setup_vreg((pdev->id + 1), (vdd ? 1 : 0)); 
			}
		}
	...
	else
	{
		if (pdev->id == 4) 
		{
			...
			if( is_err_occur == 1 )	
			{
				printk(KERN_ERR "%s: Broken SD card. Power on blocked.\n",__func__);
				goto out;			
			}
...
static unsigned int msm7x30_sdcc_slot_status(struct device *dev)
{
	...
	if( gpio_get_value_cansleep(GPIO_SD_DET) == 0 )
...
static void __init msm7x30_init_mmc(void)
{
	...
	sdcc_vreg_data[4].vreg_data = vreg_gp10;
	sdcc_vreg_data[4].level = 2850;
...
#if  	defined(CONFIG_TOUCHSCREEN_TSC2007_MODULE)
...
static void __init msm7x30_init(void)
{
	...
	i2c_register_board_info(0, msm_bd6082gcl_boardinfo,
			ARRAY_SIZE(msm_bd6082gcl_boardinfo));	
    ...
    i2c_register_board_info(4, nim_boardinfo, ARRAY_SIZE(nim_boardinfo));





	i2c_register_board_info(4 , msm_audio_boardinfo,

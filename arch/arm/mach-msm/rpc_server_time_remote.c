	...
	case RPC_TIME_TOD_SET_APPS_BASES: {
		...
		if (msmrtc_is_suspended()) {
			...
			if (now && tick_at_suspend) {
				if (now < tick_at_suspend) {
					sleep = (now + (4294967296LL * NSEC_PER_SEC / SCLK_HZ)) - tick_at_suspend;
					printk("%s : Revised a count value (now = %lld, tick_at_suspend = %lld, sleep = %lld)\n",
						__func__, now, tick_at_suspend, sleep);
				} else {

...
struct msm_chg_rpc_ids {
	...
	unsigned	chg_usb_audio_acc_det_proc;




    unsigned    chg_usb_audio_det_mic_proc;
...
static int msm_chg_init_rpc(unsigned long vers)
{
	if (((vers & RPC_VERSION_MAJOR_MASK) == 0x00010000) ||
		...
		chg_rpc_ids.chg_usb_audio_acc_det_proc 		=20;




        chg_rpc_ids.chg_usb_audio_det_mic_proc       = 21;
...
int msm_chg_usb_get_usb_audio(bool *det_usb_audio)
{
	int rc ;
	...
	rc = msm_rpc_call_reply(chg_ep, chg_rpc_ids.chg_usb_audio_acc_det_proc,
							&req, sizeof(req),
							&rsp, sizeof(rsp),
	...
	if (rc < 0) {
		...
		if (be32_to_cpu(rsp.hdr.data.acc_hdr.accept_stat) != RPC_ACCEPTSTAT_SUCCESS) {
			pr_err("----%s: RPC call was not successful (%d)\n", __func__,
					be32_to_cpu(rsp.hdr.data.acc_hdr.accept_stat));
			rc = -EINVAL;
		} else {
			if (be32_to_cpu(rsp.usb_acc) == 0) {
				*det_usb_audio = false;
			} else {
				*det_usb_audio = true;
			}
			pr_err("----%s: USB/ACC Det = %d\n", __func__, (int)(*det_usb_audio));
...
EXPORT_SYMBOL(msm_chg_usb_get_usb_audio);




int msm_chg_usb_audio_det_mic(bool *det_usb_audio_mic)
{
    int rc ;
	...
	if (!chg_ep || IS_ERR(chg_ep))
		return -EAGAIN;
	rc = msm_rpc_call_reply(chg_ep, chg_rpc_ids.chg_usb_audio_det_mic_proc,
							&req, sizeof(req),
							&rsp, sizeof(rsp),
							msecs_to_jiffies(250));
	...
	if (rc < 0) {
		...
		if (be32_to_cpu(rsp.hdr.data.acc_hdr.accept_stat) != RPC_ACCEPTSTAT_SUCCESS) {
			...
			if (be32_to_cpu(rsp.usb_mic) == 0) {
                
                
				*det_usb_audio_mic = true;
				
			} else {
                
                
				*det_usb_audio_mic = false;
				
			}
			pr_err("----%s: USB-Audio MIC Det = %d\n", __func__, (int)(*det_usb_audio_mic));
...
EXPORT_SYMBOL(msm_chg_usb_audio_det_mic);
